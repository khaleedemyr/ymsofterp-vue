...
Route::middleware('auth')->group(function () {
    ...
    Route::get('/google-review/dashboard/drilldown', [GoogleReviewController::class, 'dashboardDrilldown'])->name('google-review.dashboard.drilldown');
    ...
    Route::get('/customer-voice-command-center/pending-capa-verifications', [CustomerVoiceCommandCenterController::class, 'pendingCapaVerificationsJson'])
        ->name('customer-voice-command-center.pending-capa-verifications');
    Route::get('/customer-voice-command-center/cases/{id}/brief', [CustomerVoiceCommandCenterController::class, 'caseBriefJson'])
        ->name('customer-voice-command-center.cases.brief');
    ...
    Route::delete('/customer-voice-command-center/cases/{id}/capa', [CustomerVoiceCommandCenterController::class, 'destroyCapa'])
        ->name('customer-voice-command-center.cases.capa.destroy');
...
// Sub Category resource route
    ...
    Route::get('/kpi-evaluations/{kpiEvaluation}/export-pdf', [KpiEvaluationController::class, 'exportPdf'])->name('kpi-evaluations.export-pdf');
    Route::get('/kpi-evaluations/{kpiEvaluation}/outlet-breakdowns', [KpiEvaluationController::class, 'bulkOutletBreakdowns'])->name('kpi-evaluations.outlet-breakdowns');
    Route::get('/kpi-evaluations/{kpiEvaluation}/items/{item}/outlet-breakdown', [KpiEvaluationController::class, 'itemOutletBreakdown'])->name('kpi-evaluations.items.outlet-breakdown');
...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::resource('overtime-submissions', \App\Http\Controllers\OvertimeSubmissionController::class)->only(['index', 'create', 'store', 'show', 'edit', 'update', 'destroy']);
...
Route::get('/cost-report/official-cost-detail', [\App\Http\Controllers\CostReportController::class, 'officialCostDetail'])->name('cost-report.official-cost-detail');
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/api/contra-bon/po-gr-detail', [\App\Http\Controllers\ContraBonController::class, 'getPOGRDetail']);
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::post('/warehouse-transfer/validate-serial', [\App\Http\Controllers\WarehouseTransferController::class, 'validateSerialForWT'])->name('warehouse-transfer.validate-serial');
...
Route::get('/outlet-serial-receive/create', [\App\Http\Controllers\OutletSerialReceiveController::class, 'create'])->name('outlet-serial-receive.create');
Route::post('/outlet-serial-receive', [\App\Http\Controllers\OutletSerialReceiveController::class, 'store'])->name('outlet-serial-receive.store');
...
Route::get('/outlet-internal-use-waste/stock-cut-items', [\App\Http\Controllers\OutletInternalUseWasteController::class, 'getStockCutItems'])->name('outlet-internal-use-waste.stock-cut-items');
Route::get('/outlet-internal-use-waste/search-items-with-stock', [\App\Http\Controllers\OutletInternalUseWasteController::class, 'searchItemsWithStock'])->name('outlet-internal-use-waste.search-items-with-stock');
...
Route::get('/api/warehouse-cost-health-dashboard/transaction-detail', [App\Http\Controllers\WarehouseCostHealthDashboardController::class, 'transactionDetail'])->name('warehouse-cost-health-dashboard.transaction-detail');
...
Route::get('/outlet-mac-tracking', [App\Http\Controllers\OutletMacTrackingController::class, 'index'])->name('outlet-mac-tracking.index');
Route::get('/api/outlet-mac-tracking/options', [App\Http\Controllers\OutletMacTrackingController::class, 'options'])->name('outlet-mac-tracking.options');
Route::get('/api/outlet-mac-tracking', [App\Http\Controllers\OutletMacTrackingController::class, 'data'])->name('outlet-mac-tracking.data');
...
Route::post('qa2-audits/{id}/update-auditees', [\App\Http\Controllers\Qa2AuditController::class, 'updateAuditees'])->name('qa2-audits.update-auditees');
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::prefix('web-profile/home-service-packages')->name('web-profile.home-service-packages.')->group(function () {
        ...
        Route::get('/create', [\App\Http\Controllers\WebProfileController::class, 'homeServicePackagesCreate'])->name('create');
        Route::post('/', [\App\Http\Controllers\WebProfileController::class, 'homeServicePackagesStore'])->name('store');
        Route::get('/{id}/edit', [\App\Http\Controllers\WebProfileController::class, 'homeServicePackagesEdit'])->name('edit');
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::post('/crm/omnichannel-teams/message-templates/upload-media', [App\Http\Controllers\OmnichannelMessageTemplateController::class, 'uploadMedia'])->name('crm.omnichannel-teams.message-templates.upload-media');
...
Route::post('/payroll/finance-report/pay', [App\Http\Controllers\PayrollFinanceReportController::class, 'pay'])->name('payroll.finance-report.pay');
Route::post('/payroll/finance-report/rollback-payment', [App\Http\Controllers\PayrollFinanceReportController::class, 'rollbackPayment'])->name('payroll.finance-report.rollback-payment');
Route::get('/payroll/rekap', [App\Http\Controllers\PayrollRecapController::class, 'index'])->name('payroll.rekap');
Route::get('/payroll/rekap/export', [App\Http\Controllers\PayrollRecapController::class, 'export'])->name('payroll.rekap.export');
...
Route::middleware(['auth', 'verified'])->prefix('jabatan-training')->name('jabatan-training.')->group(function () {
  ...
  // Attendance Routes
  Route::middleware(['auth', 'verified'])->group(function () {
    Route::get('/attendance-tracking', [\App\Http\Controllers\AttendanceTrackingController::class, 'index'])->name('attendance-tracking.index');
    Route::get('/api/attendance-tracking/outlet-detail', [\App\Http\Controllers\AttendanceTrackingController::class, 'outletDetail'])->name('api.attendance-tracking.outlet-detail');
...
Route::middleware(['auth', 'verified'])->prefix('api/approval')->group(function () {
    ...
    Route::get('/pending-supervisor-queue', [\App\Http\Controllers\ApprovalController::class, 'getPendingSupervisorQueue'])->name('api.approval.pending-supervisor-queue');
...
// Laporan void bill / item / order dari POS (HO + outlet)
Route::get('/pos-void-bill-report', [\App\Http\Controllers\PosVoidItemRequestController::class, 'reportPage'])->name('pos-void-bill.report');
Route::get('/pos-void-bill-report/export', [\App\Http\Controllers\PosVoidItemRequestController::class, 'exportReport'])->name('pos-void-bill.report.export');
...
Route::middleware(['auth', 'verified'])->prefix('just-academy')->name('just-academy.')->group(function () {
    ...
    Route::post('/my-training/{schedule}/check-in', [\App\Http\Controllers\JustAcademy\MyTrainingController::class, 'checkIn'])->name('my-training.check-in');
    ...
    Route::post('/my-training/{schedule}/quizzes/{quizId}/start', [\App\Http\Controllers\JustAcademy\MyTrainingController::class, 'startQuiz'])->name('my-training.quizzes.start');

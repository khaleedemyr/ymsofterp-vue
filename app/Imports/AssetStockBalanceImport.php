        ...
        try {
            foreach ($rows as $index => $row) {
                ...
                try {
                    ...
                    if (!isset($row['warehouse_outlet_id']) || $row['warehouse_outlet_id'] === '' || $row['warehouse_outlet_id'] === null) $missingFields[] = 'Warehouse Outlet ID';
                    ...
                    if (!$item) {
                        $inactiveItem = Item::where('sku', $row['sku'])
                            ...
                            ->first();
                        if ($inactiveItem) {
                            throw new \Exception("Item '{$row['name']}' (SKU: {$row['sku']}) ditemukan tapi status tidak aktif atau bukan asset");
                        } else {
                    ...
                    $warehouseOutlet = DB::table('warehouse_outlets')
                        ->where('id', $row['warehouse_outlet_id'])
                        ->first();
                    ...
                    DB::table('asset_inventory_cards')->insert([
                        ...
                        'saldo_value' => $value,
                        'description' => $row['notes'] ?? 'Initial Stock Balance Asset',
                        'created_at' => now(),
                        'updated_at' => now(),
                    ]);
                    ...
                    $this->errorCount++;
                    \Log::error('Import Saldo Awal Stock Asset Error: ' . $e->getMessage(), [
                        'row' => $index + 2,
                        'data' => $row
                    ]);
                }

...
use App\Exports\NpdServiceCalibrationReportExport;
...
class NpdServiceCalibrationReportController extends Controller
{
    ...
    {
        return Inertia::render('Report/NpdServiceCalibrationReport', [
    ...
    public function apiExport(Request $request)
    {
        try {
            ...
            $filename = sprintf(
                'npd_service_calibration_report_%s_%s_%s.xlsx',
                $validated['date_from'],
                $validated['date_to'],
                now()->format('Ymd_His')
            );

            return Excel::download(new NpdServiceCalibrationReportExport($data), $filename);
        } catch (ValidationException $e) {
            return response()->json([
                'success' => false,
                'message' => collect($e->errors())->flatten()->first() ?? 'Validasi gagal.',

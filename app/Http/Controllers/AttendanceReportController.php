...
class AttendanceReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (!empty($outletId) || !empty($divisionId) || !empty($search) || !empty($bulan) || !empty($tahun)) {
            ...
            $processedData = $this->groupAttendanceScansByUserDay($rawData);
...
/**
     ...
     * @return array<string,float>
     */
    private function batchOnePlusOneHoursByUserDate(array $userIds, string $startDate, string $endDate): array
    {
        if ($userIds === []) {
            return [];
        }

        $items = OnePlusOneSubmissionItem::query()
            ->join('one_plus_one_submissions as ops', 'one_plus_one_submission_items.submission_id', '=', 'ops.id')
            ->whereNull('ops.deleted_at')
            ->whereIn('one_plus_one_submission_items.user_id', $userIds)
            ->whereBetween('one_plus_one_submission_items.one_plus_one_date', [$startDate, $endDate])
            ->get([
                'one_plus_one_submission_items.user_id',
                'one_plus_one_submission_items.one_plus_one_date',
                'one_plus_one_submission_items.deduction_hours',
            ]);

        $result = [];
        foreach ($items as $item) {
            $key = $this->overtimeMapKey((int) $item->user_id, (string) $item->one_plus_one_date);
            $result[$key] = ($result[$key] ?? 0) + (float) $item->deduction_hours;
        }
    ...
    private function resolveFinalOvertimeHours(
        float $actualOvertimeHours,
        ?float $requestedOvertimeHours,
        ?float $onePlusOneHours
    ): float {
        $effective = $this->resolveEffectiveOvertimeHours($actualOvertimeHours, $requestedOvertimeHours);
        $deduction = max(0, $onePlusOneHours ?? 0);

        return max(0, $effective - $deduction);
    }
                ...
                foreach ($employeeGroups as $userId => $employeeRows) {
                    ...
                    try {
                        ...
                        $dailyAttendance = $employeeRows->map(function($row) use ($firstRow, $overtimeRequestedByUserDate, $extraOffByUserDate, $onePlusOneByUserDate) {
                            ...
                            $finalTotal = floor($this->resolveFinalOvertimeHours(
                                $actualTotal,
                                $overtimeRequestedByUserDate[$overtimeKey] ?? null,
                                $onePlusOneForDate
                            ));
    ...
    {
        if (! empty($processedRow->last_outlet_id) && ! empty($processedRow->user_id)) {
            $outletShift = DB::table('user_shifts as us')
                ->leftJoin('shifts as s', 'us.shift_id', '=', 's.id')
                ->where('us.user_id', $processedRow->user_id)
                ->where('us.tanggal', $tanggal)
                ->where('us.outlet_id', $processedRow->last_outlet_id)
                ->select('s.time_start', 's.time_end', 's.shift_name', 'us.shift_id')
                ->first();
    ...
    /**
     * Kelompokkan raw scan att_log per user per hari (termasuk outlet scan).
     *
     * @param  \Illuminate\Support\Collection<int, object>  $rawData
     * @return array<string, array<string, mixed>>
     */
    private function groupAttendanceScansByUserDay($rawData, array $extraFields = []): array
    {
        $processedData = [];
        ...
        foreach ($rawData as $scan) {
            ...
            if (! isset($processedData[$key])) {
                $processedData[$key] = array_merge([
                    'tanggal' => $date,
                    'user_id' => $scan->user_id,
                    'nama_lengkap' => $scan->nama_lengkap ?? null,
                    'scans' => [],
                ], $extraFields);
            }
    ...
    private function workTimelineService(): AttendanceWorkTimelineService
    {
     ...
     */
    private function calculateEarlyCheckoutLateness($jamKeluar, $shiftEnd, $isCrossDay = false, ?string $tanggal = null) {
        if (!$jamKeluar || !$shiftEnd) {
            return 0;
        }
        ...
        if ($tanggal) {
            ...
            $diffSeconds = strtotime($shiftEndDateTime) - strtotime($jamKeluar);

...
class StockCutController extends Controller
{
    ...
    public function calculateMenuCost(Request $request)
    {
        ...
        // Ambil order_items individual untuk menghitung modifier qty dengan benar
        $orderItemsIndividual = DB::table('order_items')
            ->join('items', 'order_items.item_id', '=', 'items.id')
            ->whereDate('order_items.created_at', $tanggal)
            ->where('order_items.kode_outlet', $qr_code)
            ->where('order_items.stock_cut', 1)
            ->select('order_items.item_id', 'order_items.qty', 'order_items.modifiers', 'items.type')
            ->get();
        
        // Collect all modifiers separately dari order_items individual
        $allModifiersData = [];
        foreach ($orderItemsIndividual as $oiIndividual) {
            if ($oiIndividual->modifiers) {
                $modifiers = json_decode($oiIndividual->modifiers, true);
                if (is_array($modifiers)) {
                    foreach ($modifiers as $group) {
                        if (is_array($group)) {
                            foreach ($group as $modifierName => $modifierQty) {
                                        ...
                                        foreach ($modifierBom as $bomItem) {
                                            ...
                                            if (isset($bomItem['item_id']) && isset($bomItem['qty']) && isset($bomItem['unit_id'])) {
                                                ...
                                                if ($inventoryItem) {
                                                    // Tentukan warehouse berdasarkan type item
                                                    $warehouse = null;
                                                    if (in_array($oiIndividual->type, ['Food Asian', 'Food Western', 'Food'])) {
                                                        $warehouse = DB::table('warehouse_outlets')
                                                            ->where('outlet_id', $id_outlet)
                                                            ->where('name', 'Kitchen')
                                                            ->where('status', 'active')
                                                            ->first();
                                                    } elseif ($oiIndividual->type == 'Beverages') {
                                                        $warehouse = DB::table('warehouse_outlets')
                                                            ->where('outlet_id', $id_outlet)
                                                            ->where('name', 'Bar')
                                                            ->where('status', 'active')
        ...
        foreach ($allModifiersData as $modifierKey => $modifierData) {
            ...
            // Ambil category dari modifier option
            $modifierOption = DB::table('modifier_options')
                ->where('name', $modifierData['modifier_name'])
                ->first();
        ...
        // Group menu costs by category
        $menuCostsGrouped = [];
        foreach ($menuCosts as $menu) {
            $categoryName = $menu['category_name'] ?: 'Tanpa Kategori';
            if (!isset($menuCostsGrouped[$categoryName])) {
                $menuCostsGrouped[$categoryName] = [];
            }
            $menuCostsGrouped[$categoryName][] = $menu;
        }
        
        // Group modifier costs by category
        $modifierCostsGrouped = [];
        foreach ($modifierCosts as $modifier) {
            $categoryName = $modifier['category_name'] ?: 'Tanpa Kategori';
            if (!isset($modifierCostsGrouped[$categoryName])) {
                $modifierCostsGrouped[$categoryName] = [];
            }
            $modifierCostsGrouped[$categoryName][] = $modifier;
        }
        ...
        return response()->json([
            ...
            'menu_costs_grouped' => $menuCostsGrouped,
            'modifier_costs' => $modifierCosts,
            'modifier_costs_grouped' => $modifierCostsGrouped,
    ...
    public function getVarianceReport(Request $request)
    {
        ...
        $canManage = $this->userCanManageStockCutVariance($user);
        $rows = $query->orderByDesc('scv.created_at')
            ->skip(($page - 1) * $perPage)
            ->take($perPage)
            ->get()
            ->map(function ($row) use ($canManage) {
                ...
                $row->can_adjust = $canManage && $row->status === 'open';
                $row->can_rollback = $canManage && $this->isStockCutVarianceAdjustRollbackable($row);
                $row->adjust_reference_id = $this->stockCutVarianceAdjustReferenceId($row);
    ...
    /**
     * Balancing stok minus ke 0 + tulis kartu IN + tutup variance Open (tanpa approval).
     */
    public function adjustVariance(Request $request, $id)
    {
        if (! \Schema::hasTable('stock_cut_variances')) {
            return response()->json([
                'status' => 'error',
        ...
        try {
            ...
            $message = $result['did_stock_in']
                ...
                : 'Stok sudah tidak minus; variance Open ditutup tanpa stock in.';
            ...
            \Log::error('adjustVariance failed', [
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'Gagal adjust: '.$e->getMessage(),
            ], 500);
        }
    }
    ...
    public function rollbackVarianceAdjust(Request $request, $id)
    {
        ...
        try {
            ...
            return response()->json([
                'status' => 'success',
                'message' => 'Rollback Adjust berhasil. Variance dibuka kembali'
                    .($result['qty_reversed'] > 0 ? ' dan stok IN dibatalkan.' : '.'),
                'data' => $result,
            ]);
        } catch (\RuntimeException $e) {
            return response()->json([
            ...
            \Log::error('rollbackVarianceAdjust failed', [
                'variance_id' => $varianceId,
                'error' => $e->getMessage(),
            ]);
    ...
    private function userCanManageStockCutVariance($user): bool
    {
        ...
        return ($user->id_role === '5af56935b011a')
            || ((int) ($user->division_id ?? 0) === 11);
    }

    private function isStockCutVarianceAdjustRollbackable(object $row): bool
    {
        return $row->status === 'closed'
            && ($row->closed_via ?? null) === 'adjustment'
            && ($row->closed_reference_type ?? null) === 'stock_cut_variance_adjustment'
            && ! empty($row->closed_reference_id);
    }

    private function stockCutVarianceAdjustReferenceId(object $row): ?int
    {
        if (! $this->isStockCutVarianceAdjustRollbackable($row)) {
            return null;
        }

        return (int) $row->closed_reference_id;
    }

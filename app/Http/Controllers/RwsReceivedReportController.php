...
use App\Exports\RwsReceivedReportExport;
use App\Services\RwsRetailFoodReconciliationService;
use Illuminate\Http\Request;
use Inertia\Inertia;
use Maatwebsite\Excel\Facades\Excel;

class RwsReceivedReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (! $loadData) {
            return Inertia::render('RetailWarehouseSale/ReceivedReport', [
                'rows' => [],
                'by_outlet' => [],
                'summary' => [
                    'total_rws' => 0,
        ...
        $result = $this->reconciliation->matchedRws([
            ...$filters,
            'include_items' => true,
        ]);
    ...
    public function export(Request $request)
    {
        ...
        $filename = sprintf(
            'RWS_Sudah_Diterima_%s_%s_%s.xlsx',
            $filters['from_date'],
            $filters['to_date'],
            now()->format('Ymd_His')
        );

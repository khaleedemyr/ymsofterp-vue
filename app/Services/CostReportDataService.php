...
class CostReportDataService
{
    ...
    public function loadOutletWarehouseStockRows(array $outletIds, array $warehouseOutletIds)
    {
    ...
    public function computeBeginInventoryTotalMacByWarehouse(
        $stockRows,
        array $outletIds,
    ...
    public function computeEndingInventoryTotalMacByWarehouse(
        $stockRows,
        array $outletIds,
        array $warehouseOutletIds,
    ...
    public function buildCategoryCostRows($outlets, array $reportRows, string $tanggalAwalBulan, string $tanggalAkhirBulan): array
    {
     ...
     */
    public function getReportData(string $bulan): array
    {
     ...
     */
    public function computeBeginInventoryTotalMac(
        int $outletId,
     ...
     */
    public function computeEndingInventoryTotalMac(
        int $outletId,
        int $warehouseOutletId,
     ...
     */
    public function computeCostRndByOutlet(string $tanggalAwal, string $tanggalAkhir): array
    {
     ...
     */
    public function computeCategoryCostByOutlet(string $tanggalAwal, string $tanggalAkhir): array
    {
        $aggregates = $this->getInternalUseWasteAggregates($tanggalAwal, $tanggalAkhir);
     ...
     * Return array [ outlet_id => total ].
     */
    public function computeMealEmployeesByOutlet(string $tanggalAwal, string $tanggalAkhir): array
    {
     ...
     * Return array [ outlet_id => total ].
     */
    public function computeGuestSuppliesByOutlet(string $tanggalAwal, string $tanggalAkhir): array
    {
     ...
     * Return array [ outlet_id => total ].
     */
    public function computeSpoilageByOutlet(string $tanggalAwal, string $tanggalAkhir): array
    {
     ...
     * Return array [ outlet_id => total ].
     */
    public function computeWasteByOutlet(string $tanggalAwal, string $tanggalAkhir): array
    {
     ...
     * Return array [ outlet_id => total ].
     */
    public function computeNonCommodityByOutlet(string $tanggalAwal, string $tanggalAkhir): array
    {
     ...
     * Return array [ outlet_id => total ].
     */
    public function computeCategoryCostTypeByOutlet(string $type, string $tanggalAwal, string $tanggalAkhir): array
    {
        $aggregates = $this->getInternalUseWasteAggregates($tanggalAwal, $tanggalAkhir);
     ...
     */
    public function getInternalUseWasteAggregates(string $tanggalAwal, string $tanggalAkhir): array
    {
     ...
     */
    public function computeOutletTransferByOutlet(string $tanggalAwal, string $tanggalAkhir): array
    {
        $outletIds = DB::table('tbl_data_outlet')
            ->where('is_outlet', 1)
            ->where('status', 'A')
     ...
     */
    public function computeSalesBeforeDiscountByOutlet(string $tanggalAwal, string $tanggalAkhir): array
    {
     ...
     */
    public function computeDiscountByOutlet(string $tanggalAwal, string $tanggalAkhir): array
    {
        $rows = DB::table('orders')
            ->join('tbl_data_outlet as o', 'orders.kode_outlet', '=', 'o.qr_code')
            ->where('o.is_outlet', 1)
     ...
     */
    public function computeCogsStockCutByOutlet(string $tanggalAwal, string $tanggalAkhir): array
    {
     ...
     */
    public function sumCogsAktualForOutlets(array $outletIds, string $periodMonth): ?float
    {
        if (empty($outletIds) || !preg_match('/^\d{4}-\d{2}$/', $periodMonth)) {
            return null;
        }

        $rows = $this->getCachedCostInventoryRows($periodMonth);
        $byOutlet = collect($rows)->keyBy('outlet_id');

        $total = 0.0;
        $found = false;
        foreach ($outletIds as $outletId) {
            $row = $byOutlet->get((int) $outletId);
            if ($row !== null) {
                $total += (float) ($row['cogs_aktual'] ?? 0);
                $found = true;
            }
        }
     ...
     * @param  list<int>  $outletIds
     */
    public function sumCategoryCostTypeForOutlets(array $outletIds, string $periodMonth, string $type): ?float
    {
        if (empty($outletIds) || !preg_match('/^\d{4}-\d{2}$/', $periodMonth)) {
            return null;
        }

        $bulanCarbon = Carbon::parse($periodMonth . '-01');
        $tanggalAwal = $bulanCarbon->format('Y-m-01');
        $tanggalAkhir = $bulanCarbon->format('Y-m-t');
        $byOutlet = $this->computeCategoryCostTypeByOutlet($type, $tanggalAwal, $tanggalAkhir);

        $total = 0.0;
        $found = false;
        foreach ($outletIds as $outletId) {
            $outletId = (int) $outletId;
            if (array_key_exists($outletId, $byOutlet)) {
                $total += (float) $byOutlet[$outletId];
                $found = true;
            }
        }
    ...
    public function getCachedCostInventoryRows(string $bulan): array
    {
        $bulanCarbon = Carbon::parse($bulan . '-01');
        $bulanSebelumnya = $bulanCarbon->copy()->subMonth();
        $tanggalAkhirBulanSebelumnya = $bulanSebelumnya->format('Y-m-t');

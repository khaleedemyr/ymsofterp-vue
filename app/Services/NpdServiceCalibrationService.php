...
use App\Models\NpdServiceCalibrationParticipant;
use App\Models\NpdServiceCalibrationProduct;
use App\Models\NpdServiceCalibrationResult;
use App\Models\User;
use Illuminate\Support\Facades\DB;

class NpdServiceCalibrationService
{
    /** @var list<string> */
    ...
    public const ALL_PARAMETER_CODES = self::PARAMETER_CODES;
    ...
    public function parameterOptions(): array
    {
        $labels = [
            ...
            'menu_explanation' => 'Menu explanation',
            'suggestive_selling' => 'Suggestive selling',
            'production_presentation' => 'Production Presentation',
            'serving_standard' => 'Serving Standard (SOP)',
            'handling_guest_question' => 'Handling Guest Question',
        ];
    ...
    public function calendarEvents(int $year, int $month): array
    {
        ...
        $records = NpdServiceCalibration::query()
            ->with(['products'])
        ...
        return $records->map(function (NpdServiceCalibration $record) {
     ...
     */
    public function syncProducts(NpdServiceCalibration $calibration, array $products): void
    {
        NpdServiceCalibrationProduct::where('calibration_id', $calibration->id)->delete();

        foreach ($products as $index => $product) {
            NpdServiceCalibrationProduct::create([
    ...
    public function saveConduct(
        NpdServiceCalibration $calibration,
        array $participants,
        array $results
    ): void {
        NpdServiceCalibrationParticipant::where('calibration_id', $calibration->id)->delete();
        NpdServiceCalibrationResult::where('calibration_id', $calibration->id)->delete();
        ...
        foreach ($participants as $index => $participant) {
            ...
            $participantModels[(int) $user->id] = NpdServiceCalibrationParticipant::create([
                'calibration_id' => $calibration->id,
                'user_id' => $user->id,
        ...
        foreach ($results as $row) {
            ...
            NpdServiceCalibrationResult::create($payload);
        }
    ...
    public function notifyConductor(NpdServiceCalibration $calibration): void
    {
        $calibration->load('products');
        ...
        NotificationService::create([
            ...
            'type' => 'npd_service_calibration_assigned',
            'title' => 'NPD Service Calibration',
            'message' => sprintf(
                'Anda ditugaskan conduct NPD Service Calibration di %s pada %s. Produk: %s',
                $calibration->outlet_name,
                $dateLabel,
                $productList ?: '-'
            ),
            'url' => config('app.url').'/npd-service-calibration/'.$calibration->id,
            'is_read' => 0,
        ]);
    }
     ...
     */
    public function buildConductPayload(NpdServiceCalibration $calibration): array
    {
    ...
    public function buildReport(
        ...
        $calibrations = NpdServiceCalibration::query()
     ...
     * @return array<string, mixed>
     */
    public function snapshot(NpdServiceCalibration $calibration): array
    {

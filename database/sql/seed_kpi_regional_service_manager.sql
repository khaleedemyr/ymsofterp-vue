-- =====================================================
-- KPI Template — Regional Service Manager
-- Jalankan SETELAH:
...
-- =====================================================
-- Struktur (sesuai Excel Regional Service Manager):
...
--   • Regional FB Manager Jakarta & Tangerang
--   • Regional Service Manager Bandung
-- =====================================================

START TRANSACTION;

-- Pastikan KS06 ada (dari extended seed; aman jika sudah ada)
...
SELECT
    'KPI_REGIONAL_SERVICE_MANAGER_v1',
    'Regional Service Manager',
    'Template KPI Regional Service Manager — Customer Experience, Sales & Upselling, Team Development, Compliance.',
    1,
    'draft',
    ...
    NOW()
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `kpi_templates` WHERE `code` = 'KPI_REGIONAL_SERVICE_MANAGER_v1');

UPDATE `kpi_templates`
SET
    `name` = 'Regional Service Manager',
    `description` = 'Template KPI Regional Service Manager — Customer Experience, Sales & Upselling, Team Development, Compliance.',
    `erp_data_scope` = 'all_outlets',
    `updated_at` = NOW()
WHERE `code` = 'KPI_REGIONAL_SERVICE_MANAGER_v1';

SET @tpl_id := (SELECT `id` FROM `kpi_templates` WHERE `code` = 'KPI_REGIONAL_SERVICE_MANAGER_v1' LIMIT 1);
...
WHERE j.status = 'A'
  AND (
      j.nama_jabatan LIKE '%Regional FB Manager%Jakarta%'
      OR j.nama_jabatan LIKE '%Regional FB Manager Jakarta%'
      OR j.nama_jabatan LIKE '%Regional Service Manager%Bandung%'
      OR j.nama_jabatan LIKE '%Regional Service Manager Bandung%'
  )
  AND NOT EXISTS (
...
-- Strategy bobot = total item weight per blok (22 + 14 + 34 + 30 = 100)
...
FROM (
    ...
    SELECT 'KS04', 'KPI22',     8.00, 3, '22 Person' UNION ALL
...
-- WHERE t.code = 'KPI_REGIONAL_SERVICE_MANAGER_v1'

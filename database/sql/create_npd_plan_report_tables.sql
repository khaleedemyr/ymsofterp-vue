...
CREATE TABLE IF NOT EXISTS `npd_plan_reports` (
    ...
    `status` ENUM('draft', 'submitted', 'approved', 'rejected', 'requires_revision', 'cancelled') NOT NULL DEFAULT 'draft',
    `notes` TEXT NULL,
...
CREATE TABLE IF NOT EXISTS `npd_plan_report_items` (
    ...
    `proposed_launch_area_outlet` TEXT NULL COMMENT 'JSON array [{id,name}] launch outlets',

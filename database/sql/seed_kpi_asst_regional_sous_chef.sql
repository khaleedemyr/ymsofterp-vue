-- =====================================================
-- KPI Template — Asst. Regional Sous Chef
-- Jalankan SETELAH:
--   1. seed_kpi_template_justus_sample.sql
--   2. seed_kpi_parameters_extended.sql      (KS08, KPI25/KPI27/KPI30, KPI21/KPI22)
--   3. seed_kpi_gm_operation.sql             (KPI_GM02-05/08, KPI13-KPI16)
-- Parameter TIDAK diduplikasi — reuse kode yang sudah ada.
-- =====================================================
-- Struktur (sesuai Excel Asst. Regional Sous Chef):
--   KS01 COGS & Productivity              18% → KPI_GM02, KPI_GM03, KPI_GM04, KPI_GM05
--   KS08 Kitchen Operational Excellence   20% → KPI25, KPI27
--   KS03 Customer Experience              15% → KPI_GM08, KPI30
--   KS04 Team Development                 24% → KPI13, KPI21, KPI22
--   KS05 Compliance & Team Support        23% → KPI14, KPI15, KPI16
-- =====================================================
-- Jabatan:
--   • Asst. Regional Sous Chef
-- =====================================================
...
SELECT
    'KPI_ASST_REGIONAL_SOUS_CHEF_v1',
    'Asst. Regional Sous Chef',
    'Template KPI Asst. Regional Sous Chef — COGS, Food Quality, Customer Experience, Team Development, Compliance.',
    1,
    'draft',
    ...
    NOW()
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `kpi_templates` WHERE `code` = 'KPI_ASST_REGIONAL_SOUS_CHEF_v1');

UPDATE `kpi_templates`
SET
    `name` = 'Asst. Regional Sous Chef',
    `description` = 'Template KPI Asst. Regional Sous Chef — COGS, Food Quality, Customer Experience, Team Development, Compliance.',
    `erp_data_scope` = 'all_outlets',
    `updated_at` = NOW()
WHERE `code` = 'KPI_ASST_REGIONAL_SOUS_CHEF_v1';

SET @tpl_id := (SELECT `id` FROM `kpi_templates` WHERE `code` = 'KPI_ASST_REGIONAL_SOUS_CHEF_v1' LIMIT 1);
...
WHERE j.status = 'A'
  AND (
      j.nama_jabatan LIKE '%Asst%Regional%Sous%'
      OR j.nama_jabatan LIKE '%Asst. Regional Sous Chef%'
      OR j.nama_jabatan LIKE '%Assistant%Regional%Sous%'
      OR j.nama_jabatan LIKE '%Assistant Regional Sous Chef%'
  )
  AND j.nama_jabatan NOT LIKE '%Executive%'
  AND NOT EXISTS (
...
FROM (
    SELECT 'KS01' AS code, 18.00 AS weight, 0 AS sort_order UNION ALL
    SELECT 'KS08', 20.00, 1 UNION ALL
    SELECT 'KS03', 15.00, 2 UNION ALL
    SELECT 'KS04', 24.00, 3 UNION ALL
    SELECT 'KS05', 23.00, 4
) v
...
FROM (
    ...
    SELECT 'KS01', 'KPI_GM04',  5.00, 2, '<= 5%' UNION ALL
    ...
    SELECT 'KS03', 'KPI_GM08', 10.00, 0, '<= 24 hours' UNION ALL
    SELECT 'KS03', 'KPI30',     5.00, 1, '<= 0.50%' UNION ALL
    -- KS04 Team Development (24%)
    SELECT 'KS04', 'KPI13',  8.00, 0, '>= 9 Person & 100% on Time' UNION ALL
    SELECT 'KS04', 'KPI21',  8.00, 1, '100% on Time' UNION ALL
    SELECT 'KS04', 'KPI22',  8.00, 2, '>= 2 Person' UNION ALL
    -- KS05 Compliance & Team Support (23%)
    SELECT 'KS05', 'KPI14', 10.00, 0, '100%' UNION ALL
    SELECT 'KS05', 'KPI15',  8.00, 1, '>= 80%' UNION ALL
...
JOIN (
    ...
    SELECT 'KS04', 'KPI13', 0 UNION ALL SELECT 'KS04', 'KPI21', 1 UNION ALL SELECT 'KS04', 'KPI22', 2 UNION ALL
...
-- WHERE t.code = 'KPI_ASST_REGIONAL_SOUS_CHEF_v1'

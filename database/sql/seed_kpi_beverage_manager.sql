-- =====================================================
-- KPI Template — Beverage Manager
-- Jalankan SETELAH:
--   1. seed_kpi_template_justus_sample.sql   (D001-D024, KPI01-KPI16)
--   2. seed_kpi_parameters_extended.sql      (KS09, KPI25/KPI27/KPI29, KPI21/KPI22)
--   3. seed_kpi_gm_operation.sql             (KPI_GM08, KPI12-KPI16)
-- Parameter TIDAK diduplikasi — reuse kode yang sudah ada.
-- =====================================================
-- Struktur (sesuai Excel Beverage Manager):
--   KS09 Beverage Operational Excellence  25% → KPI25, KPI27
--   KS03 Customer Experience              15% → KPI_GM08, KPI29
--   KS04 Team Development                 35% → KPI12, KPI13, KPI21, KPI22
--   KS05 Compliance & Team Support        25% → KPI14, KPI15, KPI16
-- =====================================================
-- Jabatan:
--   • Beverage Manager (bukan Regional / Asst / R&D)
-- =====================================================
...
SELECT
    ...
    'Beverage Manager',
    'Template KPI Beverage Manager — Beverage Ops Excellence, Customer Experience, Team Development, Compliance.',
    1,
    'draft',
    ...
    NOW()
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `kpi_templates` WHERE `code` = 'KPI_BEVERAGE_MANAGER_v1');

UPDATE `kpi_templates`
SET
    `name` = 'Beverage Manager',
    `description` = 'Template KPI Beverage Manager — Beverage Ops Excellence, Customer Experience, Team Development, Compliance.',
    `erp_data_scope` = 'all_outlets',
    `updated_at` = NOW()
WHERE `code` = 'KPI_BEVERAGE_MANAGER_v1';

SET @tpl_id := (SELECT `id` FROM `kpi_templates` WHERE `code` = 'KPI_BEVERAGE_MANAGER_v1' LIMIT 1);
...
WHERE j.status = 'A'
  AND j.nama_jabatan LIKE '%Beverage Manager%'
  AND j.nama_jabatan NOT LIKE '%Regional%'
  AND j.nama_jabatan NOT LIKE '%Asst%'
  AND j.nama_jabatan NOT LIKE '%Assistant%'
  AND j.nama_jabatan NOT LIKE '%R&D%'
  AND j.nama_jabatan NOT LIKE '%R%26%D%'
  AND NOT EXISTS (
...
FROM (
    SELECT 'KS09' AS code, 25.00 AS weight, 0 AS sort_order UNION ALL
...
FROM (
    ...
    SELECT 'KS04', 'KPI12',    15.00, 0, '>= 90%' UNION ALL
    SELECT 'KS04', 'KPI13',    10.00, 1, '>= 2 Person & 100% on Time' UNION ALL
    SELECT 'KS04', 'KPI21',     5.00, 2, '100% on Time' UNION ALL
    SELECT 'KS04', 'KPI22',     5.00, 3, '>= 2 Person' UNION ALL
    -- KS05 Compliance & Team Support (25%)
    SELECT 'KS05', 'KPI14',    10.00, 0, '>= 90%' UNION ALL
    SELECT 'KS05', 'KPI15',    10.00, 1, '>= 90%' UNION ALL
...
-- WHERE t.code = 'KPI_BEVERAGE_MANAGER_v1'

-- Create table for Stock Opname Header
CREATE TABLE IF NOT EXISTS `outlet_stock_opnames` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `opname_number` varchar(50) NOT NULL COMMENT 'Format: SO-YYYYMMDD-XXX',
...
-- Create table for Stock Opname Items (Detail)
CREATE TABLE IF NOT EXISTS `outlet_stock_opname_items` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `fk_opname_items_opname` FOREIGN KEY (`stock_opname_id`) REFERENCES `outlet_stock_opnames` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_opname_items_inventory` FOREIGN KEY (`inventory_item_id`) REFERENCES `outlet_food_inventory_items` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Create table for Stock Opname Approval Flows
CREATE TABLE IF NOT EXISTS `outlet_stock_opname_approval_flows` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `stock_opname_id` bigint(20) unsigned NOT NULL,
  ...
  CONSTRAINT `fk_opname_flows_opname` FOREIGN KEY (`stock_opname_id`) REFERENCES `outlet_stock_opnames` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_opname_flows_approver` FOREIGN KEY (`approver_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

...
$csvPath = $exportDir . DIRECTORY_SEPARATOR . 'orders_discrepancy_from_2026-06-26.csv';
$fp = fopen($csvPath, 'w');
...
if (!empty($diffRows)) {
    echo PHP_EOL . "Contoh 30 baris pertama:" . PHP_EOL;
    $sample = array_slice($diffRows, 0, 30);
    foreach ($sample as $i => $item) {
        $n = $i + 1;
        echo sprintf(
            "%d) %s | %s | %s | GT: %.2f | EXP: %.2f | SELISIH: %.2f\n",
            $n,
            $item['tanggal'],
            $item['outlet'],
            $item['nomor_order'],
            $item['grand_total'],
            $item['expected_simple'],
            $item['selisih_simple']
        );
    }
}

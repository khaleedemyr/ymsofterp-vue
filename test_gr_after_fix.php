...
echo "=== TEST GR NUMBERS SETELAH PERBAIKAN ===\n\n";
...
if ($contraBon->sources && $contraBon->sources->count() > 0) {
    ...
    foreach ($contraBon->sources as $source) {
        ...
        echo "  - PurchaseOrder loaded: " . ($source->relationLoaded('purchaseOrder') ? 'YES' : 'NO') . "\n";
        
        if ($source->source_type === 'purchase_order' && $source->purchaseOrder) {
            $po = $source->purchaseOrder;
            echo "  ✅ PurchaseOrder ditemukan: {$po->number}\n";
            ...
            if ($source->gr_id) {
                ...
                if ($grNumber) {
                    ...
                    echo "  ❌ GR ID {$source->gr_id} tidak ditemukan\n";
                }
            } else {
            ...
            if ($source->source_type === 'purchase_order') {
                echo "  ❌ PurchaseOrder TIDAK ditemukan atau tidak ter-load\n";
            }
        }
        echo "\n";
    }
    ...
    // Cek semua GR yang ada
    $grNumbers = array_filter($finalSourceNumbers, function($num) {
        return strpos($num, 'GR-') === 0;
    });
    
    if (count($grNumbers) > 0) {
        echo "\nGR Numbers yang ditemukan: " . implode(', ', $grNumbers) . "\n";
    } else {
        echo "\n⚠️  TIDAK ADA GR Numbers yang ditemukan!\n";
    }
} else {
    echo "❌ Contra Bon tidak punya sources\n";
}

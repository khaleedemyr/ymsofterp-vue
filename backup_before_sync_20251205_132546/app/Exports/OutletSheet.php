...
class OutletSheet implements FromCollection, WithHeadings, WithMapping, WithStyles, WithColumnWidths
{
    private $report;
    private $tanggal;
    ...
    public function collection()
    {
        try {
            ...
            // Prepare outlet data with totals
            $outletData = [];
            ...
            // Add total row for outlets
            if ($data->isNotEmpty()) {
                $outletData[] = [
                    'customer' => 'TOTAL OUTLET',
            ...
            return collect($outletData);
        } catch (\Exception $e) {
            \Log::error('Outlet sheet collection error: ' . $e->getMessage());
            return collect([]);
        }
    }

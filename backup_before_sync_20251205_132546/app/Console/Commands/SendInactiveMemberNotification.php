...
class SendInactiveMemberNotification extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'member:notify-inactive';
     ...
     * @var string
     */
    protected $description = 'Send notification to members who haven\'t made a transaction in the last 7 days';
    ...
    {
        $this->info('Checking for inactive members (no transaction in 7 days)...');

        $now = Carbon::now();
        $sevenDaysAgo = $now->copy()->subDays(7)->startOfDay();
        $eightDaysAgo = $now->copy()->subDays(8)->startOfDay();

        // Find members who:
        // 1. Are active
        // 2. Allow notifications
        // 3. Have at least one transaction (to avoid notifying new members)
        // 4. Last transaction was exactly 7 days ago (with 1 day window: 7-8 days ago)
        //    This ensures we send notification once, approximately 7 days after last transaction
        ...
        ", [
            $eightDaysAgo->toDateString(),
            $sevenDaysAgo->toDateString()
        ]);

        if (empty($members)) {
            $this->info('No inactive members found (no transaction in 7 days).');
            Log::info('No inactive members found for notification', [
                'date_range_start' => $eightDaysAgo->toDateString(),
                'date_range_end' => $sevenDaysAgo->toDateString(),
            ]);
            return 0;
        }

        $this->info("Found " . count($members) . " member(s) with no transaction in 7 days.");
        ...
        foreach ($members as $memberData) {
            try {
                ...
                // Verify last transaction is still 7 days ago (to avoid duplicate notifications)
                ...
                // Only send if exactly 7 days (with 1 day window: 7-8 days)
                // This ensures we send notification once, approximately 7 days after last transaction
                if ($daysSinceLastTransaction < 7 || $daysSinceLastTransaction > 8) {
                    $this->warn("Member {$member->id} last transaction was {$daysSinceLastTransaction} days ago, not in 7-8 day window, skipping...");
                    continue;
                }

                $this->info("Sending inactive member notification to member {$member->id} ({$member->nama_lengkap})");

                $result = $fcmService->sendToMember(
                    $member,
                    'We Miss You! 💙',
                    'How was your experience today? Your feedback helps us serve you better.',
                    [
                ...
                if ($result['success_count'] > 0) {
                    ...
                    Log::info('Inactive member notification sent', [
                        'member_id' => $member->id,
                    ...
                    Log::warning('Failed to send inactive member notification', [
                        'member_id' => $member->id,
                        'member_name' => $member->nama_lengkap,
                        'result' => $result,
                    ]);
                ...
                Log::error('Error sending inactive member notification', [
        ...
        Log::info('Inactive member notifications summary', [

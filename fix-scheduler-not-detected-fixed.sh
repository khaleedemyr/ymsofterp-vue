...
# Usage: bash fix-scheduler-not-detected-fixed.sh
...
echo "   OK Laravel artisan ditemukan"
echo ""
...
if [ $SCHEDULE_COUNT -eq 0 ]; then
    echo "   WARNING: Tidak ada scheduled tasks ditemukan di Kernel.php"
    echo "   Check file: app/Console/Kernel.php"
else
    echo "   OK Scheduled tasks ditemukan di code"
fi
echo ""
...
if [ $? -eq 0 ]; then
    ...
    echo "   OK Tidak ada syntax error"
fi
echo ""
...
if grep -q "protected function schedule" app/Console/Kernel.php; then
    echo "   OK Method schedule() ditemukan"
    
    if grep -A 5 "protected function schedule" app/Console/Kernel.php | grep -q "\$schedule->"; then
        echo "   OK Method schedule() memiliki content"
    else
        echo "   WARNING: Method schedule() kosong atau tidak ada scheduled tasks"
    fi
else
...
if grep -q "namespace App\\\\Console" app/Console/Kernel.php && grep -q "class Kernel" app/Console/Kernel.php; then
    echo "   OK Namespace dan class benar"
else
    echo "   WARNING: Check namespace dan class di Kernel.php"
fi
echo ""

echo "10. Checking autoload..."
if [ -f "composer.json" ]; then
    echo "   OK composer.json ditemukan"
    if command -v composer &> /dev/null; then
        echo "   Running: composer dump-autoload"
        composer dump-autoload 2>&1 | tail -5
    else
        echo "   WARNING: Composer tidak ditemukan, skip autoload"
    fi
else
    echo "   WARNING: composer.json tidak ditemukan"
fi
echo ""
...
if [ $SCHEDULE_COUNT -eq 0 ]; then
    ...
    echo "OK Scheduled tasks ada di code ($SCHEDULE_COUNT tasks)"
    echo ""

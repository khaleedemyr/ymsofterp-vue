-- Query Rekap FJ Fleksibel - Cek data dengan parameter yang bisa disesuaikan

-- Ganti parameter di bawah ini sesuai kebutuhan:
-- SET @outlet_id = 23;        -- ID outlet yang ingin dicek
-- SET @tahun = 2024;          -- Tahun
-- SET @bulan = 9;             -- Bulan (1-12)

-- Atau gunakan query tanpa parameter untuk melihat semua data yang tersedia

-- 1. Query untuk outlet tertentu dengan rentang tanggal yang lebih luas
SELECT 
    'outlet_food_good_receives' as source_table,
    DATE(gr.receive_date) as tanggal,
    o.id_outlet,
    o.nama_outlet as customer,
    o.is_outlet,
    -- Main Kitchen
    SUM(CASE 
        WHEN w.name IN ('MK1 Hot Kitchen', 'MK2 Cold Kitchen') 
        THEN i.received_qty * COALESCE(fo.price, 0) 
        ELSE 0 
    END) as main_kitchen,
    
    -- Main Store
    SUM(CASE 
        WHEN w.name = 'MAIN STORE' AND sc.name NOT IN ('Chemical', 'Stationary', 'Marketing') 
        THEN i.received_qty * COALESCE(fo.price, 0) 
        ELSE 0 
    END) as main_store,
    
    -- Chemical
    SUM(CASE 
        WHEN w.name = 'MAIN STORE' AND sc.name = 'Chemical' 
        THEN i.received_qty * COALESCE(fo.price, 0) 
        ELSE 0 
    END) as chemical,
    
    -- Stationary
    SUM(CASE 
        WHEN w.name = 'MAIN STORE' AND sc.name = 'Stationary' 
        THEN i.received_qty * COALESCE(fo.price, 0) 
        ELSE 0 
    END) as stationary,
    
    -- Marketing
    SUM(CASE 
        WHEN w.name = 'MAIN STORE' AND sc.name = 'Marketing' 
        THEN i.received_qty * COALESCE(fo.price, 0) 
        ELSE 0 
    END) as marketing,
    
    -- Line Total
    SUM(i.received_qty * COALESCE(fo.price, 0)) as line_total

FROM outlet_food_good_receives as gr
    JOIN outlet_food_good_receive_items as i ON gr.id = i.outlet_food_good_receive_id
    JOIN items as it ON i.item_id = it.id
    JOIN sub_categories as sc ON it.sub_category_id = sc.id
    JOIN units as u ON i.unit_id = u.id
    JOIN delivery_orders as do ON gr.delivery_order_id = do.id
    LEFT JOIN food_floor_order_items as fo ON (
        i.item_id = fo.item_id AND 
        fo.floor_order_id = do.floor_order_id
    )
    LEFT JOIN warehouse_division as wd ON it.warehouse_division_id = wd.id
    LEFT JOIN warehouses as w ON wd.warehouse_id = w.id
    JOIN tbl_data_outlet as o ON gr.outlet_id = o.id_outlet

WHERE 
    o.id_outlet = 23  -- Ganti dengan outlet ID yang diinginkan
    AND YEAR(gr.receive_date) = 2025  -- Ganti tahun
    AND MONTH(gr.receive_date) = 9    -- Ganti bulan
    AND gr.deleted_at IS NULL

GROUP BY DATE(gr.receive_date), o.id_outlet, o.nama_outlet, o.is_outlet

UNION ALL

-- 2. Query untuk good_receive_outlet_suppliers
SELECT 
    'good_receive_outlet_suppliers' as source_table,
    DATE(gr.receive_date) as tanggal,
    o.id_outlet,
    o.nama_outlet as customer,
    o.is_outlet,
    -- Main Kitchen
    SUM(CASE 
        WHEN w.name IN ('MK1 Hot Kitchen', 'MK2 Cold Kitchen') 
        THEN i.qty_received * COALESCE(fo.price, 0) 
        ELSE 0 
    END) as main_kitchen,
    
    -- Main Store
    SUM(CASE 
        WHEN w.name = 'MAIN STORE' AND sc.name NOT IN ('Chemical', 'Stationary', 'Marketing') 
        THEN i.qty_received * COALESCE(fo.price, 0) 
        ELSE 0 
    END) as main_store,
    
    -- Chemical
    SUM(CASE 
        WHEN w.name = 'MAIN STORE' AND sc.name = 'Chemical' 
        THEN i.qty_received * COALESCE(fo.price, 0) 
        ELSE 0 
    END) as chemical,
    
    -- Stationary
    SUM(CASE 
        WHEN w.name = 'MAIN STORE' AND sc.name = 'Stationary' 
        THEN i.qty_received * COALESCE(fo.price, 0) 
        ELSE 0 
    END) as stationary,
    
    -- Marketing
    SUM(CASE 
        WHEN w.name = 'MAIN STORE' AND sc.name = 'Marketing' 
        THEN i.qty_received * COALESCE(fo.price, 0) 
        ELSE 0 
    END) as marketing,
    
    -- Line Total
    SUM(i.qty_received * COALESCE(fo.price, 0)) as line_total

FROM good_receive_outlet_suppliers as gr
    JOIN good_receive_outlet_supplier_items as i ON gr.id = i.good_receive_id
    JOIN items as it ON i.item_id = it.id
    JOIN sub_categories as sc ON it.sub_category_id = sc.id
    JOIN units as u ON i.unit_id = u.id
    LEFT JOIN warehouse_division as wd ON it.warehouse_division_id = wd.id
    LEFT JOIN warehouses as w ON wd.warehouse_id = w.id
    LEFT JOIN delivery_orders as do ON gr.delivery_order_id = do.id
    LEFT JOIN food_floor_order_items as fo ON (
        i.item_id = fo.item_id AND 
        fo.floor_order_id = do.floor_order_id
    )
    JOIN tbl_data_outlet as o ON gr.outlet_id = o.id_outlet

WHERE 
    o.id_outlet = 23  -- Ganti dengan outlet ID yang diinginkan
    AND YEAR(gr.receive_date) = 2025  -- Ganti tahun
    AND MONTH(gr.receive_date) = 9    -- Ganti bulan

GROUP BY DATE(gr.receive_date), o.id_outlet, o.nama_outlet, o.is_outlet

ORDER BY tanggal, source_table;

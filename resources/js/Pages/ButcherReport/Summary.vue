        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gradient-to-r from-blue-50 to-blue-100">
            <tr>
              ...
              <th class="px-4 py-2 text-right text-xs font-bold text-blue-700 uppercase tracking-wider">Qty</th>
              <th class="px-4 py-2 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">Satuan</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!data.length">
              <td colspan="4" class="text-center py-10 text-gray-400">Tidak ada data hasil butcher.</td>
            </tr>
            <tr v-for="row in data" :key="row.process_date + row.item_name">
              ...
              <td class="px-4 py-2 text-right">{{ formatNumber(row.result_qty) }}</td>
...
function applyFilter() {
  router.get(route('butcher-summary-report.index'), { from: from.value, to: to.value }, { preserveState: true, replace: true });
}

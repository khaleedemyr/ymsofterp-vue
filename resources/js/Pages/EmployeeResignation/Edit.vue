<template>
  <AppLayout title="Edit Employee Resignation">
    <div class="w-full py-8 px-4">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <i class="fa-solid fa-user-minus text-red-500"></i> Edit Employee Resignation
        </h1>
        <Link
...
const form = useForm({
  outlet_id: props.resignation.outlet_id || '',
  employee_id: props.resignation.employee_id || null,
  resignation_date: props.resignation.resignation_date ? props.resignation.resignation_date.split(' ')[0] : '',
  resignation_type: props.resignation.resignation_type || '',
  notes: props.resignation.notes || '',
  approvers: props.resignation.approval_flows ? props.resignation.approval_flows.map(flow => ({
    id: flow.approver?.id,
    nama_lengkap: flow.approver?.nama_lengkap,
    email: flow.approver?.email,
    jabatan: flow.approver?.jabatan
  })).sort((a, b) => {
    const flowA = props.resignation.approval_flows.find(f => f.approver?.id === a.id);
    const flowB = props.resignation.approval_flows.find(f => f.approver?.id === b.id);
    return (flowA?.approval_level || 0) - (flowB?.approval_level || 0);
  }) : [],
})
...
// Initialize employee options and selected employee
onMounted(() => {
  ...
  if (form.employee_id) {
    const emp = employeeOptions.value.find(e => e.id === form.employee_id)
    if (emp) {
...
const loadEmployees = async () => {
  ...
  try {
    ...
    // If current employee is in the new list, keep it selected
    if (form.employee_id) {
      const emp = employeeOptions.value.find(e => e.id === form.employee_id)
      if (emp) {
        selectedEmployee.value = emp
...
const submitForm = () => {
  ...
  form.put(`/employee-resignations/${props.resignation.id}`, {
    onSuccess: () => {
      Swal.fire({
        icon: 'success',
        title: 'Success!',
        text: 'Employee resignation updated successfully!',
        confirmButtonText: 'OK'
      })
    },
    onError: (errors) => {
      console.error('Failed to update resignation:', errors)
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: errors.message || 'Failed to update employee resignation'
      })
    }
  })
}

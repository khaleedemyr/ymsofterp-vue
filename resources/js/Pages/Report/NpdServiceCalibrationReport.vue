  ...
  <AppLayout>
    <div class="max-w-[1600px] mx-auto py-8 px-4">
      <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2 mb-6">
        ...
        NPD Service Calibration Report
      </h1>
        ...
        <table class="fbc-report-table min-w-[1500px] w-full text-xs border-collapse">
          <thead>
            <tr>
              ...
              <th :colspan="activeNpdParameterOptions.length * 2" class="fbc-th-group">
                CALIBRATION PARAMETER
              </th>
            </tr>
            <tr>
              <th
                v-for="param in activeNpdParameterOptions"
                :key="param.code"
                colspan="2"
                class="fbc-th-param"
              >
            ...
            <tr>
              <template v-for="param in activeNpdParameterOptions" :key="`${param.code}-sub`">
          ...
          <tbody>
            <tr v-for="row in report.rows" :key="`${row.no}-${row.employee_name}-${row.product_name}`">
              ...
              <template v-for="param in activeNpdParameterOptions" :key="`${row.no}-${param.code}`">
...
const activeNpdParameterOptions = computed(() => {
...
async function fetchReport() {
  ...
  try {
    ...
    const res = await axios.get('/api/report/npd-service-calibration', { params });
...
function exportExcel() {
  ...
  window.open(`/report/npd-service-calibration/export?${query.toString()}`, '_blank');
}
</script>

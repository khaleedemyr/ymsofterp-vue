    ...
    <div class="max-w-7xl w-full mx-auto py-8 px-2">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <i class="fa-solid fa-wrench text-blue-500"></i> Asset Maintenances
        </h1>
      ...
      <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-if="maintenances.data && maintenances.data.length === 0">
                <td colspan="8" class="px-6 py-4 text-center text-gray-500">
                  Tidak ada data
                </td>
              </tr>
              <tr v-for="(item, index) in maintenances.data" :key="item.id" class="hover:bg-gray-50">
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  {{ (maintenances.current_page - 1) * maintenances.per_page + index + 1 }}
                </td>
        ...
        <!-- Pagination -->
        <div v-if="maintenances.last_page > 1" class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
          <div class="flex items-center justify-between">
            <div class="flex-1 flex justify-between sm:hidden">
              <button
                @click="goToPage(maintenances.prev_page_url)"
              ...
              <button
                @click="goToPage(maintenances.next_page_url)"
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p class="text-sm text-gray-700">
                  Menampilkan
                  <span class="font-medium">{{ maintenances.from }}</span>
                  sampai
                  <span class="font-medium">{{ maintenances.to }}</span>
                  dari
...
const debouncedSearch = debounce(() => {
  router.get('/asset-management/maintenances', {
    search: search.value,
...
function openCreate() {
  router.visit('/asset-management/maintenances/create');
}

function viewMaintenance(item) {
  router.visit(`/asset-management/maintenances/${item.id}`);
}

function openEdit(item) {
  router.visit(`/asset-management/maintenances/${item.id}/edit`);
}

async function completeMaintenance(item) {
  const result = await Swal.fire({
    title: 'Complete Maintenance?',
    text: `Konfirmasi bahwa maintenance untuk "${item.asset?.name}" sudah selesai?`,
    icon: 'question',
  ...
  if (result.isConfirmed) {
    try {
      const response = await axios.post(`/asset-management/maintenances/${item.id}/complete`, {}, {
        headers: {
...
async function hapus(item) {
  const result = await Swal.fire({
    ...
    text: `Maintenance record akan dihapus`,
    icon: 'warning',
    showCancelButton: true,
  ...
  if (result.isConfirmed) {
    try {
      const response = await axios.delete(`/asset-management/maintenances/${item.id}`, {
        headers: {
          'X-Requested-With': 'XMLHttpRequest',
          'Accept': 'application/json',
        },
      ...
      if (response.data?.success || response.status === 200) {
        Swal.fire('Berhasil', 'Maintenance berhasil dihapus', 'success');
        reload();
      }
    } catch (error) {
      Swal.fire('Error', 'Gagal menghapus maintenance', 'error');
    }
  }
}
...
watch([assetId, maintenanceType, status, dateFrom, dateTo, perPage], () => {
  router.get('/asset-management/maintenances', {
    search: search.value,
    asset_id: assetId.value,
    maintenance_type: maintenanceType.value,

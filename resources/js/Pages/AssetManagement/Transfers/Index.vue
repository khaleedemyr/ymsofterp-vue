    ...
    <div class="max-w-7xl w-full mx-auto py-8 px-2">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <i class="fa-solid fa-exchange-alt text-blue-500"></i> Asset Transfers
        </h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
          <div>
            ...
            <input
              ...
              v-model="search"
              @input="onSearchInput"
              placeholder="Cari asset code atau nama..."
      ...
      <div class="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">From Outlet</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">To Outlet</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-for="(item, index) in transfers.data" :key="item.id" class="hover:bg-gray-50">
        ...
        <!-- Pagination -->
        <div v-if="transfers.last_page > 1" class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
          <div class="flex items-center justify-between">
            <div class="flex-1 flex justify-between sm:hidden">
              <button
                @click="goToPage(transfers.prev_page_url)"
              ...
              <button
                @click="goToPage(transfers.next_page_url)"
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p class="text-sm text-gray-700">
                  Menampilkan
                  <span class="font-medium">{{ transfers.from }}</span>
                  sampai
                  <span class="font-medium">{{ transfers.to }}</span>
                  dari
...
const debouncedSearch = debounce(() => {
  router.get('/asset-management/transfers', {
    search: search.value,
...
function openCreate() {
  router.visit('/asset-management/transfers/create');
}

function viewTransfer(item) {
  router.visit(`/asset-management/transfers/${item.id}`);
}

async function approveTransfer(item) {
  const result = await Swal.fire({
    title: 'Approve Transfer?',
    text: `Transfer asset "${item.asset?.name}" dari ${item.from_outlet?.name} ke ${item.to_outlet?.name}?`,
    icon: 'question',
  ...
  if (result.isConfirmed) {
    try {
      const response = await axios.post(`/asset-management/transfers/${item.id}/approve`, {}, {
        headers: {
...
async function rejectTransfer(item) {
  const { value: reason } = await Swal.fire({
    ...
    inputAttributes: {
      'aria-label': 'Masukkan alasan rejection'
    },
    showCancelButton: true,
  ...
  if (reason) {
    try {
      const response = await axios.post(`/asset-management/transfers/${item.id}/reject`, {
...
async function completeTransfer(item) {
  const result = await Swal.fire({
    ...
    text: `Konfirmasi bahwa asset sudah dipindahkan ke ${item.to_outlet?.name}?`,
    icon: 'question',
    showCancelButton: true,
    confirmButtonColor: '#3085d6',
    cancelButtonColor: '#d33',
  ...
  if (result.isConfirmed) {
    try {
      const response = await axios.post(`/asset-management/transfers/${item.id}/complete`, {}, {
        headers: {
          'X-Requested-With': 'XMLHttpRequest',
          'Accept': 'application/json',
        },
...
watch([assetId, outletId, status, perPage], () => {
  router.get('/asset-management/transfers', {
    search: search.value,
    asset_id: assetId.value,
    outlet_id: outletId.value,

  ...
  <AppLayout>
    <div class="max-w-7xl w-full mx-auto py-8 px-2">
      <div class="mb-6 flex justify-between items-center">
        <div>
          <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <i class="fa-solid fa-store text-blue-500"></i> Asset by Outlet Report
          </h1>
          <p class="text-gray-600 mt-1">Asset per outlet</p>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div v-for="outlet in outletSummary" :key="outlet.id" class="bg-white rounded-xl shadow-lg p-4">
          <h3 class="text-sm font-medium text-gray-500 mb-2">{{ outlet.name }}</h3>
          <p class="text-2xl font-bold text-gray-900">{{ outlet.count }}</p>
...
const outletSummary = computed(() => {
  ...
  props.assets.forEach(asset => {
    const outletId = asset.current_outlet_id || 'unassigned';
    const outletName = asset.current_outlet?.name || 'Tidak Terikat';
    if (!summary[outletId]) {
      summary[outletId] = { id: outletId, name: outletName, count: 0 };
    }
    summary[outletId].count++;
  });
  return Object.values(summary);
});

function applyFilters() {
  router.get('/asset-management/reports/asset-by-outlet', {
...
function exportReport() {
  ...
  window.open(`/asset-management/reports/asset-by-outlet?${params.toString()}`, '_blank');
}

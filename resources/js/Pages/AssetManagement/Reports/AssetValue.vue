      ...
      <div class="mb-6 flex justify-between items-center">
        <div>
          <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <i class="fa-solid fa-dollar-sign text-blue-500"></i> Asset Value Report
          </h1>
          <p class="text-gray-600 mt-1">Nilai asset berdasarkan kategori dan outlet</p>
        </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-for="asset in assets" :key="asset.id" class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-blue-600">
                  {{ formatCurrency(asset.depreciation?.current_value || asset.purchase_price) }}
                </td>
              </tr>
            </tbody>
...
const valueType = ref(props.filters?.value_type || 'purchase');

const totalPurchaseValue = computed(() => {
  return props.assets.reduce((sum, a) => sum + (parseFloat(a.purchase_price) || 0), 0);
});

const totalCurrentValue = computed(() => {
  return props.assets.reduce((sum, a) => {
    const currentValue = a.depreciation?.current_value || a.purchase_price;
    return sum + (parseFloat(currentValue) || 0);
  }, 0);
});

function applyFilters() {
  router.get('/asset-management/reports/asset-value', {
...
function exportReport() {
  const params = new URLSearchParams({
    category_id: categoryId.value || '',
    outlet_id: outletId.value || '',
    value_type: valueType.value || 'purchase',
    export: '1',
  });
  window.open(`/asset-management/reports/asset-value?${params.toString()}`, '_blank');
}

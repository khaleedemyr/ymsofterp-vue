    ...
    <div class="w-full py-8 px-4 md:px-6 lg:px-8">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <h1 class="text-2xl font-bold flex items-center gap-2 text-gray-900">
          ...
          Pelacakan Anomali MAC Warehouse
        </h1>
      ...
      <template v-if="activeTab === 'scan'">
        <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6 text-sm text-blue-900">
          ...
          <ul class="list-disc pl-5 space-y-1 text-blue-800">
            ...
            <li><strong>Scan Anomali</strong> hanya menampilkan MAC yang <em>dicurigai bermasalah</em> (minus, lonjakan besar, value yatim, dll.).</li>
            <li>Sumber data: <code class="text-xs bg-blue-100 px-1 rounded">food_inventory_cost_histories</code> + stok <code class="text-xs bg-blue-100 px-1 rounded">food_inventory_stocks</code> (Main Store / MK).</li>
            ...
            <li>Mulai dengan <strong>Semua warehouse</strong> + rentang tanggal lebih lebar. Kalau kosong, coba preset <strong>Sensitif</strong>.</li>
          </ul>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            <div>
              ...
              <select v-model="scanFilters.warehouse_id" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div class="flex flex-wrap gap-2 mb-4">
            ...
            <button type="button" @click="applyPreset('mk')" class="text-xs px-3 py-1 rounded-full border border-red-300 bg-red-50 text-red-800 hover:bg-red-100">MK strict (max 100rb)</button>
          </div>
        ...
        <div v-if="anomalies.length > 0" class="bg-white rounded-2xl border border-gray-200 shadow-sm">
          ...
          <div class="overflow-x-auto">
            <table class="w-full min-w-[1300px] divide-y divide-gray-200 text-sm">
              <thead class="bg-gray-50">
                <tr>
                  <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tanggal</th>
                  <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Warehouse</th>
              ...
              <tbody class="divide-y divide-gray-100">
                <tr v-for="row in anomalies" :key="row.history_id || `${row.item_id}-${row.warehouse_id}-stock`" class="hover:bg-gray-50">
                  <td class="px-3 py-3">{{ row.date || '-' }}</td>
                  <td class="px-3 py-3 font-medium">{{ row.warehouse_name }}</td>
                  <td class="px-3 py-3">
                    <div class="font-medium">{{ row.item_name }}</div>
                    <div v-if="row.item_code" class="text-xs text-gray-500">{{ row.item_code }}</div>
                    <div v-if="row.qty_small !== undefined" class="text-xs text-gray-500">qty {{ row.qty_small }} · value {{ row.stock_value || '-' }}</div>
                  </td>
                  <td class="px-3 py-3">
        ...
        <div v-if="scanLoading" class="bg-white rounded-2xl border p-8 text-center text-gray-500">
          ...
          <p>Memindai anomali MAC warehouse...</p>
        </div>
      </template>
      ...
      <template v-if="activeTab === 'detail'">
        <div class="bg-indigo-50 border border-indigo-200 rounded-xl p-4 mb-6 text-sm text-indigo-900">
          <p><strong>Detail per Barang</strong> — pilih <em>Warehouse + Barang</em>, lalu klik <strong>Lihat Riwayat MAC</strong>.</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
          ...
          <table class="w-full min-w-[1200px] text-sm divide-y">
            ...
            <tbody class="divide-y">
              <tr v-for="row in macChanges" :key="row.history_id" class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3">
                  <div>{{ row.transaction_number || row.reference_label || '-' }}</div>
                  <div class="text-xs text-gray-500">{{ row.reference_type }} #{{ row.reference_id }}</div>
                </td>
                <td class="px-4 py-3">
                  <a v-if="row.source_url" :href="row.source_url" target="_blank" class="text-blue-600 hover:underline text-xs">
      ...
      <template v-if="activeTab === 'modules'">
        <div class="bg-white rounded-2xl border shadow-sm p-6">
          <p class="text-sm text-gray-600 mb-6">Daftar modul ERP yang menulis perubahan MAC ke <code class="text-xs bg-gray-100 px-1 rounded">food_inventory_cost_histories</code> (warehouse HO / MK).</p>
...
const filters = ref({ warehouse_id: '', item_id: '' })
...
const emptyStateMessage = computed(() => {
  ...
  if (inPeriod === 0 && total === 0) {
    return 'Warehouse yang dipilih belum punya data di food_inventory_cost_histories. Coba warehouse lain atau perlebar tanggal.'
  }
  if (inPeriod === 0 && total > 0) {
    return `Ada ${total} riwayat MAC di luar periode tanggal. Perlebar rentang tanggal atau klik preset "Rentang 6 bulan".`
  }
  return 'MAC dalam periode ini terlihat normal menurut filter. Turunkan threshold atau cek riwayat per barang di tab Detail.'
})
...
function applyPreset(name) {
  if (name === 'sensitive') {
    ...
    scanFilters.value.min_spike_percent = 50
    scanFilters.value.spike_multiplier = 2
    scanFilters.value.max_mac = 100000
  }
...
async function loadWarehouses() {
  try {
    const res = await axios.get('/api/warehouse-mac-anomaly-tracking/options')
...
async function runScan(page = 1) {
  ...
  scanFilters.value.date_from = maxDate(scanFilters.value.date_from, historyCutoffDate)
  try {
    const res = await axios.get('/api/warehouse-mac-anomaly-tracking/scan', {
      params: {
        warehouse_id: scanFilters.value.warehouse_id || undefined,
...
async function openDetailForRow(row) {
  ...
  filters.value.warehouse_id = String(row.warehouse_id)
...
async function loadData(page = 1) {
  const missing = []
  if (!filters.value.warehouse_id) missing.push('Warehouse')
  ...
  try {
    const res = await axios.get('/api/warehouse-mac-anomaly-tracking', {
      params: {
...
function typeLabel(type) {
  const map = {
    ...Object.fromEntries(anomalyTypeOptions.map(o => [o.value, o.label])),

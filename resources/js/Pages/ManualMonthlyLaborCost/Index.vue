    ...
    <div class="max-w-7xl mx-auto py-8 px-4">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">Input manual nilai Labor Cost per outlet per periode</p>
        </div>
        <Link
          :href="route('manual-monthly-labor-cost.create')"
            ...
            <tr v-for="row in records.data" :key="row.id" class="border-b hover:bg-gray-50">
              ...
              <td class="px-4 py-3">
                <div class="flex justify-center gap-2">
                  <Link
                    :href="route('manual-monthly-labor-cost.show', row.id)"
                  ...
                  <Link
                    :href="route('manual-monthly-labor-cost.edit', row.id)"
...
function applyFilters() {
  router.get(route('manual-monthly-labor-cost.index'), {
...
async function confirmDelete(row) {
  ...
  router.delete(route('manual-monthly-labor-cost.destroy', row.id))
}
</script>

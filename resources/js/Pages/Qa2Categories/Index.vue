...
const reload = debounce(() => {
  router.get(route('qa2-categories.index'), {
    search: search.value,
    status: status.value,
...
function openCreate() {
  router.visit(route('qa2-categories.create'));
}

function openEdit(row) {
  router.visit(route('qa2-categories.edit', row.id));
}

async function deactivate(row) {
  const result = await Swal.fire({
    title: 'Nonaktifkan category?',
  ...
  router.delete(route('qa2-categories.destroy', row.id));
}
</script>

<template>
  <AppLayout title="QA2 Categories">
    <div class="w-full max-w-[90rem] mx-auto py-8 px-3">
      <div class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-2xl font-bold text-gray-800">QA2 Categories</h1>
          <p class="text-sm text-gray-500">Master kategori untuk audit QA2.</p>
        </div>
      ...
      <div class="bg-white rounded-2xl shadow overflow-hidden">
        <table class="min-w-full text-sm">
          ...
          <tbody>
            <tr v-for="row in categories.data" :key="row.id" class="border-b hover:bg-blue-50">
            ...
            <tr v-if="!categories.data?.length">
              <td colspan="4" class="text-center py-8 text-gray-400">Belum ada category QA2</td>
            </tr>
          </tbody>
        </table>
      </div>

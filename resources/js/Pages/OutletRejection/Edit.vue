        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
          <div class="p-6 text-gray-900">
            ...
            <div class="flex justify-between items-center mb-6">
              <div>
                <h3 class="text-lg font-semibold">Edit Outlet Rejection</h3>
                <p class="text-gray-600">{{ rejection.number }}</p>
              </div>
            ...
            <form @submit.prevent="submitForm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                     ...
                     <div v-if="item.selected_item" class="mt-4 p-3 bg-blue-50 rounded-lg">
                       ...
                       <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm mb-3">
                         ...
                         <div v-if="item.qty_order !== undefined" class="mt-3 pt-3 border-t border-blue-200">
                           ...
                           <div class="grid grid-cols-2 md:grid-cols-5 gap-2 text-xs">
                             <div class="bg-white p-2 rounded border">
                               ...
                               <div class="text-blue-600">{{ item.qty_order || 0 }}</div>
                             </div>
                             <div class="bg-white p-2 rounded border">
                               <div class="font-medium text-gray-700">PL Qty</div>
                               <div class="text-green-600">{{ item.qty_packing_list || 0 }}</div>
                             </div>
                             <div class="bg-white p-2 rounded border">
                               <div class="font-medium text-gray-700">DO Qty</div>
                               <div class="text-orange-600">{{ item.qty_do || 0 }}</div>
                             </div>
                             <div class="bg-white p-2 rounded border">
                               <div class="font-medium text-gray-700">GR Qty</div>
                               <div class="text-purple-600">{{ item.qty_receive || 0 }}</div>
                             </div>
                             <div class="bg-white p-2 rounded border">
                               <div class="font-medium text-gray-700">Remaining</div>
                               <div class="text-red-600 font-bold">{{ (item.qty_do || 0) - (item.qty_receive || 0) }}</div>
                             </div>
                           </div>
                         </div>
              ...
              <div class="flex justify-end space-x-4">
                ...
                >
                  ...
                  {{ isSubmitting ? 'Saving...' : 'Update Rejection' }}
                </button>
              </div>
            </form>
...
const form = useForm({
  ...
  outlet_id: props.rejection.outlet_id,
  warehouse_id: props.rejection.warehouse_id,
  delivery_order_id: props.rejection.delivery_order_id,
  notes: props.rejection.notes,
  items: []
})

// Initialize items from existing rejection
const initializeItems = () => {
  form.items = props.rejection.items.map(item => ({
    item_id: item.item_id,
    item_search: item.item?.name || '',
    selected_item: item.item,
    search_results: [],
    show_dropdown: false,
    unit_id: item.unit_id,
    unit_options: item.item ? [
      { id: item.item.small_unit_id, name: item.item.small_unit?.name },
      { id: item.item.medium_unit_id, name: item.item.medium_unit?.name },
      { id: item.item.large_unit_id, name: item.item.large_unit?.name }
...
const onDeliveryOrderChange = async () => {
  if (!form.delivery_order_id) return
...
const submitForm = () => {
  ...
  form.put(route('outlet-rejections.update', props.rejection.id), {
    onSuccess: () => {

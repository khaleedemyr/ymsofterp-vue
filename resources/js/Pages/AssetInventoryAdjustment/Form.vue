...
const searchApprovers = debounce(async () => {
    try {
        const { data } = await axios.get('/asset-inventory-adjustment/approvers', {
...
function submitForm() {
    ...
    if (!form.warehouse_outlet_id) {
        Swal.fire('Error', 'Pilih warehouse outlet.', 'error');
        return;
    }
    if (!form.reason.trim()) {
        Swal.fire('Error', 'Alasan adjustment wajib diisi.', 'error');
        return;
    }
    ...
    for (const item of form.items) {
        ...
        if (form.type === 'out' && item.qty > item.stock_small) {
    ...
    form.post('/asset-inventory-adjustments', {
        onSuccess: () => {
            Swal.fire('Berhasil', 'Adjustment berhasil dibuat.', 'success');
        },
        onError: (errors) => {
            const msg = Object.values(errors).flat().join('\n');
            Swal.fire('Error', msg || 'Gagal membuat adjustment.', 'error');
        }
    });
}
</script>
    ...
    <AppLayout>
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex items-center gap-3 mb-6">
                ...
                <h1 class="text-2xl font-bold text-teal-700">Buat Asset Stock Adjustment</h1>
            </div>

            <!-- Type Toggle -->
            <div class="bg-white rounded-xl shadow p-6 mb-6">
                <h2 class="text-lg font-semibold text-gray-800 mb-4"><i class="fa-solid fa-sliders text-teal-500 mr-2"></i>Tipe Adjustment</h2>
                <div class="flex gap-4">
                    <button @click="form.type = 'in'"
                        :class="form.type === 'in' ? 'bg-green-600 text-white ring-2 ring-green-300' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'"
                        class="flex-1 py-4 rounded-xl font-bold text-lg transition text-center">
                        <i class="fa-solid fa-arrow-down mr-2"></i> Stock In
                    </button>
                    <button @click="form.type = 'out'"
                        :class="form.type === 'out' ? 'bg-red-600 text-white ring-2 ring-red-300' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'"
                        class="flex-1 py-4 rounded-xl font-bold text-lg transition text-center">
                        <i class="fa-solid fa-arrow-up mr-2"></i> Stock Out
                    </button>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow p-6 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-500 mb-1">Alasan <span class="text-red-500">*</span></label>
                        <textarea v-model="form.reason" rows="2"
                            class="w-full rounded-lg border-gray-300 text-sm focus:ring-teal-500 focus:border-teal-500"
                            placeholder="Alasan adjustment (wajib)..."></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow p-6 mb-6">
                ...
                <div v-if="!form.warehouse_outlet_id" class="text-sm text-amber-600 mb-3">
                    <i class="fa-solid fa-info-circle mr-1"></i> Pilih warehouse outlet terlebih dahulu untuk mencari item.
                </div>
                            ...
                            <tr v-for="(item, idx) in form.items" :key="idx">
                                ...
                                <td class="px-3 py-2">
                                    <select v-model="item.selected_unit"
                                        class="rounded border-gray-300 text-sm focus:ring-teal-500 focus:border-teal-500">
                                ...
                                <td class="px-3 py-2">
                                    ...
                                    <span v-if="form.type === 'out' && item.qty > item.stock_small" class="text-xs text-red-500 block">
                                        Melebihi stok!
                                    </span>
                                </td>
                                <td class="px-3 py-2">
            ...
            <div class="flex justify-end gap-3">
                <button @click="router.visit('/asset-inventory-adjustments')"
                ...
                <button @click="submitForm" :disabled="form.processing"
                    ...
                    <i class="fa-solid fa-paper-plane mr-1"></i> Simpan Adjustment
                </button>
            </div>
        </div>
    </AppLayout>

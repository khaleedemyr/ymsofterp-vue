...
const applyFilters = debounce(() => {
    router.get('/asset-inventory-adjustments', {
...
function clearFilters() {
    filters.value = { search: '', from: '', to: '', status: '', type: '', outlet_id: '' };
}
...
function deleteAdjustment(id) {
    Swal.fire({
        ...
        text: 'Data adjustment akan dihapus permanen.',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            router.delete(`/asset-inventory-adjustments/${id}`, {
    ...
    <AppLayout>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <h1 class="text-2xl font-bold text-teal-700">Asset Stock Adjustment</h1>
                <Link href="/asset-inventory-adjustments/create"
                    ...
                    <i class="fa-solid fa-plus"></i> Buat Adjustment Baru
                </Link>
            </div>
            ...
            <div class="bg-white rounded-xl shadow p-5 mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
                <div>
                    ...
                    <input v-model="filters.search" type="text" placeholder="No. Adjustment..."
                        ...
                        <tr v-for="a in adjustments.data" :key="a.id" class="hover:bg-teal-50/30 transition">
                            ...
                            <td class="px-5 py-3">
                                <span :class="typeBadge(a.type)"
                                    class="px-2.5 py-1 rounded-full text-xs font-semibold">{{ typeLabel(a.type) }}</span>
                            </td>
                            <td class="px-5 py-3">
                                <span :class="statusBadge(a.status)"
                                    class="px-2.5 py-1 rounded-full text-xs font-semibold">{{ statusLabel(a.status) }}</span>
                            </td>
                            <td class="px-5 py-3 text-sm text-gray-600">{{ a.creator_name || '-' }}</td>
                            <td class="px-5 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <Link :href="`/asset-inventory-adjustments/${a.id}`"
                                        class="text-teal-600 hover:text-teal-800 text-sm font-medium">
                                        <i class="fa-solid fa-eye"></i> Lihat
                                    </Link>
                                    <button v-if="a.status === 'waiting_approval'" @click="deleteAdjustment(a.id)"
            ...
            <!-- Pagination -->
            <div v-if="adjustments.links && adjustments.links.length > 3" class="flex justify-center mt-6 gap-1">
                <button v-for="link in adjustments.links" :key="link.label"
                    :disabled="!link.url"

  ...
  <AppLayout>
    <div class="max-w-7xl mx-auto py-8 px-4">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <Link :href="route('npd-service-calibration.show', record.id)" class="text-gray-600 hover:text-gray-800">
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="flex justify-end gap-3">
          <Link :href="route('npd-service-calibration.show', record.id)" class="px-6 py-2.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700">
            Batal
          </Link>
          <button
            type="submit"
...
function searchParticipants(query) {
  ...
  participantTimer = setTimeout(async () => {
    ...
    try {
      const res = await axios.get('/api/npd-service-calibration/search-participants', { params: { q: query || '' } });
...
function submit() {
  ...
  buildPayload();
  form.post(route('npd-service-calibration.conduct.store', props.record.id));
}

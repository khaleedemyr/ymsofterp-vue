  ...
  <AppLayout>
    <div class="max-w-4xl mx-auto py-8 px-4">
      <div class="flex justify-between items-center mb-6">
        ...
        <Link :href="route('npd-service-calibration.index')" class="text-gray-600 hover:text-gray-800">
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="flex justify-end gap-3">
          <Link :href="route('npd-service-calibration.index')" class="px-6 py-2.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700">
            Batal
          </Link>
...
async function fetchProducts(query = '') {
  ...
  try {
    ...
    const res = await axios.get('/api/npd-service-calibration/search-products', {
      params: {
...
function onConductorSearch() {
  ...
  conductorTimer = setTimeout(async () => {
    ...
    try {
      const res = await axios.get('/api/npd-service-calibration/search-conductors', { params: { q } });
...
function submit() {
  ...
  if (isEdit.value) {
    form.put(route('npd-service-calibration.update', props.record.id));
  } else {
    form.post(route('npd-service-calibration.store'));
  }
}

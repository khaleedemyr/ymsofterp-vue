      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
            ...
            Detail NPD Service Calibration
          </h1>
          <p class="text-sm text-gray-500 mt-1">{{ record.outlet_name }} · {{ formatDate(record.scheduled_date) }}</p>
        </div>
        <div class="flex flex-wrap gap-2">
          <Link :href="route('npd-service-calibration.index')" class="px-4 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700">
            <i class="fa-solid fa-arrow-left mr-1"></i> Kalender
          </Link>
          <Link
            v-if="record.status !== 'completed'"
            :href="route('npd-service-calibration.edit', record.id)"
          ...
          <Link
            v-if="canConduct"
            :href="route('npd-service-calibration.conduct', record.id)"
...
function confirmDelete() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(route('npd-service-calibration.destroy', props.record.id));
    }
  });
}
</script>

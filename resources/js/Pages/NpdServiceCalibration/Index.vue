...
function goCreate(dateStr) {
  ...
  router.get(route('npd-service-calibration.create'), { date: dateStr });
}
...
function navigateMonth(delta) {
  ...
  while (m < 1) { m += 12; y -= 1; }
  while (m > 12) { m -= 12; y += 1; }
  router.get(route('npd-service-calibration.index'), { year: y, month: m }, {
    preserveState: true,
    replace: true,
...
function goThisMonth() {
  const n = new Date();
  router.get(route('npd-service-calibration.index'), {
    year: n.getFullYear(),
    month: n.getMonth() + 1,
...
function confirmDelete() {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(route('npd-service-calibration.destroy', id), {
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <i class="fa-solid fa-concierge-bell text-violet-600"></i>
            NPD Service Calibration
          </h1>
          <p class="text-sm text-gray-500 mt-1">Kalender jadwal NPD Service Calibration per outlet</p>
        </div>
        <Link
          :href="route('npd-service-calibration.create', { date: todayStr })"
      ...
      <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl overflow-hidden">
        ...
        <div class="p-5 space-y-3 text-sm">
          ...
          <div class="flex flex-wrap gap-2 pt-2">
            <Link
              :href="route('npd-service-calibration.show', detail.event?.calibration_id)"
            ...
            <Link
              ...
              :href="route('npd-service-calibration.edit', detail.event?.calibration_id)"

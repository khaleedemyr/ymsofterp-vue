              ...
              <div v-if="form.mode === 'travel_application' && selectedCategoryDetails" class="mt-3">
                ...
                <div v-if="budgetInfo" class="p-3 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <div v-if="budgetInfo.budget_type === 'GLOBAL'" class="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                    ...
                    <div class="md:col-span-4 mt-2 grid grid-cols-2 gap-2">
                      <div class="p-2 rounded bg-gray-50 border border-gray-200">
                        <p class="text-xs font-medium text-gray-700">Sisa Budget (Sebelum Input)</p>
                        <p class="text-base font-bold" :class="budgetInfo.category_remaining_amount < 0 ? 'text-red-800' : 'text-green-800'">
                          {{ formatCurrency(budgetInfo.category_remaining_amount) }}
                        </p>
                      </div>
                      <div class="p-2 rounded" :class="budgetInfo.remaining_after_current < 0 ? 'bg-red-50 border border-red-200' : 'bg-green-50 border border-green-200'">
                        <p class="text-xs font-medium" :class="budgetInfo.remaining_after_current < 0 ? 'text-red-700' : 'text-green-700'">
                          Sisa Budget (Setelah Input)
                        </p>
                        <p class="text-base font-bold" :class="budgetInfo.remaining_after_current < 0 ? 'text-red-800' : 'text-green-800'">
                          {{ formatCurrency(budgetInfo.remaining_after_current) }}
                        </p>
                        <p v-if="budgetInfo.remaining_after_current < 0" class="text-xs text-red-600 mt-1">
                          <i class="fa fa-exclamation-triangle mr-1"></i>Budget melebihi limit
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div v-else-if="budgetInfo.budget_type === 'PER_OUTLET'" class="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                    ...
                    <div class="md:col-span-4 mt-2 grid grid-cols-2 gap-2">
                      <div class="p-2 rounded bg-gray-50 border border-gray-200">
                        <p class="text-xs font-medium text-gray-700">Sisa Budget (Sebelum Input)</p>
                        <p class="text-base font-bold" :class="budgetInfo.outlet_remaining_amount < 0 ? 'text-red-800' : 'text-green-800'">
                  ...
                  <div v-if="budgetInfo.breakdown" class="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-2 text-xs">
                      ...
                      <div class="p-2 bg-white rounded border border-blue-100">
                        ...
                        <p class="text-sm font-bold text-purple-600">{{ formatCurrency(budgetInfo.breakdown.retail_non_food) }}</p>
                  ...
                  <div v-for="(category, categoryIdx) in outlet.categories" :key="categoryIdx" class="border border-green-200 rounded-lg p-4 bg-green-50">
                    ...
                    <div v-if="getCategoryDetails(outletIdx, categoryIdx)" class="mb-3">
                      ...
                      <div v-if="getCategoryBudgetInfo(outletIdx, categoryIdx)" class="p-3 bg-green-50 border border-green-200 rounded-lg">
                        ...
                        <!-- Budget Breakdown Detail -->
                        <div v-if="getCategoryBudgetInfo(outletIdx, categoryIdx).breakdown" class="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                          <h5 class="text-xs font-semibold text-blue-800 mb-2">
                            <i class="fa fa-list-ul mr-1"></i>Budget Breakdown Detail
                          </h5>
                          <div class="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
                            ...
                            <div class="p-2 bg-white rounded border border-blue-100">
                              <p class="text-blue-600 font-medium">Retail Non Food</p>
                              <p class="text-sm font-bold text-purple-600">{{ formatCurrency(getCategoryBudgetInfo(outletIdx, categoryIdx).breakdown.retail_non_food) }}</p>
                              <p class="text-xs text-gray-500 mt-1">Status: Approved</p>
                            </div>
                          </div>
                        </div>
...
const totalAmount = computed(() => {
  if (form.mode === 'kasbon') {
    ...
    return form.travel_items.reduce((sum, item) => sum + (item.subtotal || 0), 0)
  } else {
...
function getCategoryTotal(outletIdx, categoryIdx) {
  ...
  return category.items.reduce((sum, item) => sum + (item.subtotal || 0), 0)
}
...
function getOutletTotal(outletIdx) {
  ...
  return outlet.categories.reduce((sum, category) => {
    return sum + category.items.reduce((itemSum, item) => itemSum + (item.subtotal || 0), 0)
  }, 0)
}

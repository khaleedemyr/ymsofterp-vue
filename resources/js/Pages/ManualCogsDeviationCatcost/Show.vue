      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
            ...
            Detail Manual COGS, Deviation & Catcost
          </h1>
          <p class="text-sm text-gray-500 mt-1">Periode: <strong>{{ monthLabel }} {{ record.year }}</strong></p>
        </div>
        <div class="flex gap-2">
          <Link
            :href="route('manual-cogs-deviation-catcost.edit', record.id)"
        ...
        <table class="min-w-full text-sm">
          <thead class="bg-gray-800 text-white">
            <tr>
              ...
              <th class="px-4 py-3 text-right">Nilai COGS</th>
              <th class="px-4 py-3 text-right">% COGS</th>
              <th class="px-4 py-3 text-right">Nilai Deviation</th>
              <th class="px-4 py-3 text-right">% Deviation</th>
              <th class="px-4 py-3 text-right">Nilai Catcost</th>
              <th class="px-4 py-3 text-right">% Catcost</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="!record.items?.length">
              <td colspan="7" class="px-4 py-8 text-center text-gray-500">Tidak ada data outlet.</td>
            </tr>
            <tr
            ...
            >
              ...
              <td class="px-4 py-3 text-right">{{ formatNumber(item.cogs_value) }}</td>
              <td class="px-4 py-3 text-right">{{ formatPercent(item.cogs_percent) }}</td>
              <td class="px-4 py-3 text-right">{{ formatNumber(item.deviation_value) }}</td>
              <td class="px-4 py-3 text-right">{{ formatPercent(item.deviation_percent) }}</td>
              <td class="px-4 py-3 text-right">{{ formatNumber(item.catcost_value) }}</td>
              <td class="px-4 py-3 text-right">{{ formatPercent(item.catcost_percent) }}</td>
            </tr>
          </tbody>
        </table>
      </div>

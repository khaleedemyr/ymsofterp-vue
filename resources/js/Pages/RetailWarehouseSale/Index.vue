...
function confirmDelete(id, number) {
  if (confirm(`Apakah Anda yakin ingin menghapus penjualan ${number}?`)) {
    router.delete(route('retail-warehouse-sale.destroy', id), {
      preserveScroll: true,
      onSuccess: () => {
            ...
            <tr v-for="sale in sales.data" :key="sale.id" class="hover:bg-blue-50 transition shadow-sm">
              ...
              <td class="px-6 py-3">
                <div class="flex gap-2">
                  ...
                  <button v-if="props.canDelete" @click="confirmDelete(sale.id, sale.number)" class="inline-flex items-center btn btn-xs bg-red-100 text-red-800 hover:bg-red-200 rounded px-2 py-1 font-semibold transition">
                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">

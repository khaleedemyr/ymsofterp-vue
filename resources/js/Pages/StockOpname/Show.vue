    ...
    <div class="w-full py-8 px-4">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <i class="fa-solid fa-clipboard-check text-blue-500"></i> Detail Stock Opname
        </h1>
        <Link
      ...
      <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <h3 class="text-lg font-semibold text-gray-800 mb-4">Informasi Stock Opname</h3>
            <div class="space-y-2">
              <div>
              ...
              <div>
                <span class="text-sm font-medium text-gray-600">Outlet:</span>
                <span class="ml-2 text-sm text-gray-900">{{ stockOpname.outlet?.nama_outlet || '-' }}</span>
              </div>
              <div>
                <span class="text-sm font-medium text-gray-600">Warehouse Outlet:</span>
                <span class="ml-2 text-sm text-gray-900">{{ stockOpname.warehouse_outlet?.name || '-' }}</span>
              </div>
              <div>
          ...
          <div class="flex flex-col gap-3">
            <div v-if="stockOpname.status === 'DRAFT'" class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              ...
              <div class="flex flex-col gap-2">
                <Link
                  :href="route('stock-opnames.edit', stockOpname.id)"
      ...
      <div v-if="showApproveModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
          <h3 class="text-lg font-bold text-gray-800 mb-4">Approve Stock Opname</h3>
          <div class="mb-4">
      ...
      <div v-if="showRejectModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
          <h3 class="text-lg font-bold text-gray-800 mb-4">Reject Stock Opname</h3>
          <div class="mb-4">
...
async function submitForApproval() {
  ...
  try {
    await axios.post(route('stock-opnames.submit-approval', props.stockOpname.id), {
      approvers: approvers,
    });
...
async function approveStockOpname(action) {
  ...
  try {
    await axios.post(route('stock-opnames.approve', props.stockOpname.id), {
      action: action,
...
async function processStockOpname() {
  ...
  try {
    await axios.post(route('stock-opnames.process', props.stockOpname.id));
    router.reload();
  } catch (error) {
...
async function deleteStockOpname() {
  ...
  try {
    await axios.delete(route('stock-opnames.destroy', props.stockOpname.id));
    router.visit(route('stock-opnames.index'));
  } catch (error) {

    ...
    <div class="w-full py-8 px-4 md:px-6 lg:px-8">
      <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold flex items-center gap-2 text-gray-900">
          <i class="fa-solid fa-clock-rotate-left text-cyan-600"></i> Outlet MAC Tracking
        </h1>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <div>
            ...
            <select
              ...
              :disabled="!filters.id_outlet"
              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-cyan-500 disabled:bg-gray-100"
            >
          ...
          <div>
            ...
            <input
              ...
              :disabled="!filters.warehouse_outlet_id"
              class="w-full border border-gray-300 rounded-md px-3 py-2 mb-2 focus:outline-none focus:ring-2 focus:ring-cyan-500 disabled:bg-gray-100"
            >
            <select
            ...
            <p v-if="items.length > 0 && filteredItems.length === 0" class="text-xs text-gray-500 mt-1">Tidak ada barang yang cocok.</p>
          </div>
          <div>
          ...
          <div>
            ...
            >
              ...
              <option v-for="mod in referenceTypes" :key="mod.reference_type" :value="mod.reference_type">
                {{ mod.label }}
              </option>
            </select>
          </div>
        ...
        <div class="mt-4 flex justify-end">
          <button
            @click="loadData(1)"
            :disabled="loading"
            class="bg-cyan-600 disabled:bg-cyan-300 disabled:cursor-not-allowed text-white px-5 py-2 rounded-md hover:bg-cyan-700 transition"
          >
      ...
      <div v-if="selectedInfo" class="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 mb-6 w-full">
        <p class="text-sm text-gray-600">
          Outlet:
          <span class="font-semibold text-gray-900">{{ selectedInfo.outlet_name }}</span>
          • Warehouse:
      ...
      <div v-if="macChanges.length > 0" class="bg-white rounded-2xl border border-gray-200 shadow-sm w-full">
        <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200">
          ...
          <div class="flex items-center gap-2">
            ...
            <select
              ...
              class="border border-gray-300 rounded-md px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500"
            >
        ...
        <div class="overflow-x-auto w-full">
          <table class="w-full min-w-[1400px] divide-y divide-gray-200">
            <thead class="bg-gray-50 sticky top-0">
              <tr>
                ...
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Old MAC</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">New Cost</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">MAC</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Δ %</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="row in macChanges" :key="row.history_id" class="hover:bg-gray-50 transition-colors">
                <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{{ row.date || '-' }}</td>
                <td class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{{ row.created_at || '-' }}</td>
                <td class="px-4 py-3 text-sm text-gray-900">{{ row.old_cost }}</td>
                <td class="px-4 py-3 text-sm text-gray-900">{{ row.new_cost }}</td>
                <td class="px-4 py-3 text-sm font-semibold text-gray-900">{{ row.mac }}</td>
                ...
                <td class="px-4 py-3 text-sm text-gray-900">
                  ...
                  <span v-else>{{ row.transaction_number || '-' }}</span>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-if="!loading && hasSearched && macChanges.length === 0" class="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 text-center text-gray-500 w-full">
        <i class="fa-solid fa-inbox text-4xl mb-3 text-gray-400"></i>
        <p class="text-lg font-medium">Belum ada riwayat perubahan MAC untuk filter yang dipilih</p>
      </div>
...
const items = ref([])
const referenceTypes = ref(props.referenceTypes || [])
...
const pageRange = computed(() => {
  if (pagination.value.total === 0) return { start: 0, end: 0 }
...
async function loadOptions() {
  if (!filters.value.id_outlet) {
    warehouses.value = []
    items.value = []
    return
  }
  try {
    const response = await axios.get('/api/outlet-mac-tracking/options', {
      params: {
    ...
    if (response.data.status === 'success') {
      ...
      if (response.data.reference_types?.length) {
        referenceTypes.value = response.data.reference_types
      }
    } else {
      warehouses.value = []
      items.value = []
...
async function loadData(page = 1) {
  if (!filters.value.id_outlet || !filters.value.warehouse_outlet_id || !filters.value.item_id) {
    alert('Silakan pilih outlet, warehouse, dan barang terlebih dahulu')
    return
  }
  ...
  try {
    const response = await axios.get('/api/outlet-mac-tracking', {
      params: {
        id_outlet: filters.value.id_outlet,
    ...
    if (response.data.status === 'success') {
      ...
      selectedInfo.value = {
        outlet_name: response.data.outlet?.outlet_name || '-',
    ...
    console.error('Error loading outlet MAC tracking:', error)
    macChanges.value = []
    summary.value = null
    selectedInfo.value = null
    pagination.value = { current_page: 1, per_page: pagination.value.per_page, total: 0, last_page: 1 }

<template>
  <div class="relative w-full">
    <div class="flex gap-4 overflow-x-auto pb-2 scrollbar-hide snap-x">
      <div v-for="(item, idx) in evidence" :key="item.id" class="evidence-card snap-center">
        ...
        <div class="p-3 flex flex-col gap-1">
          ...
          <div class="text-xs text-blue-500 mt-1">{{ item.date }}</div>
        </div>
      </div>
    </div>
    <button v-if="evidence.length > 3" @click="scrollLeft" class="slider-btn left-2"><i class="fas fa-chevron-left"></i></button>
    <button v-if="evidence.length > 3" @click="scrollRight" class="slider-btn right-2"><i class="fas fa-chevron-right"></i></button>
  </div>
</template>
<script setup>
import { ref } from 'vue';
const props = defineProps({ evidence: Array });

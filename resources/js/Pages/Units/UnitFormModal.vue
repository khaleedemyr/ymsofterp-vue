...
watch(() => props.show, (val) => {
  if (val && props.mode === 'edit' && props.unit) {
    form.code = props.unit.code;
    form.name = props.unit.name;
    form.status = props.unit.status;
...
async function submit() {
  ...
  } else if (props.mode === 'edit' && props.unit) {
    form._method = 'PUT';
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-auto p-0 animate-fade-in">
      <div class="px-8 pt-8 pb-2">
        <div class="flex items-center gap-2 mb-6">
          ...
          <h3 class="text-2xl font-bold text-gray-900">{{ mode === 'edit' ? 'Edit' : 'Tambah' }} Unit</h3>
        </div>

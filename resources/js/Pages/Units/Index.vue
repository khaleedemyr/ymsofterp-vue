...
const debouncedSearch = debounce(() => {
  router.get('/units', { search: search.value, status: showInactive.value ? 'inactive' : 'active' }, { preserveState: true, replace: true });
}, 400);

watch(showInactive, (val) => {
  router.get('/units', { search: search.value, status: val ? 'inactive' : 'active' }, { preserveState: true, replace: true });
});
...
async function hapus(unit) {
  const result = await Swal.fire({
    title: 'Hapus Unit?',
    text: `Yakin ingin menghapus unit "${unit.name}"?`,
    icon: 'warning',
  ...
  router.delete(route('units.destroy', unit.id), {
    onSuccess: () => Swal.fire('Berhasil', 'Unit berhasil dihapus!', 'success'),
  });
}
...
function toggleStatus(unit) {
  ...
  router.patch(route('units.toggle-status', unit.id), { status: newStatus }, {
    preserveState: true,
    onSuccess: reload,
  });
}
    ...
    <div class="max-w-7xl w-full mx-auto py-8 px-2">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <i class="fa-solid fa-ruler text-blue-500"></i> Units
        </h1>
        ...
        <table class="w-full min-w-full divide-y divide-gray-200">
          ...
          <tbody>
            <tr v-if="units.data.length === 0">
              <td colspan="4" class="text-center py-10 text-gray-400">Tidak ada data unit.</td>
            </tr>
            <tr v-for="unit in units.data" :key="unit.id" class="hover:bg-blue-50 transition shadow-sm">
              <td class="px-6 py-3 font-mono font-semibold text-blue-700">{{ unit.code }}</td>
              <td class="px-6 py-3 font-semibold">{{ unit.name }}</td>
              <td class="px-6 py-3">
                <button
                  :class="unit.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'"
              ...
              <td class="px-6 py-3">
                <div class="flex gap-2">
                  <button @click="openEdit(unit)" class="inline-flex items-center btn btn-xs bg-yellow-100 text-yellow-800 hover:bg-yellow-200 rounded px-2 py-1 font-semibold transition">
                  ...
                  <button @click="hapus(unit)" class="inline-flex items-center btn btn-xs bg-red-100 text-red-700 hover:bg-red-200 rounded px-2 py-1 font-semibold transition">

...
const form = useForm({
  ...
  nilai_dasar_potongan_bpjs_kesehatan: 0,
  nilai_dasar_potongan_bpjs_ketenagakerjaan: 0,
  id_bpjs_kategori: '',
  nilai_point: 0,
  status: 'A',
});
...
watch(() => props.show, (val) => {
  if (val && props.mode === 'edit' && props.dataLevel) {
    ...
    form.nilai_dasar_potongan_bpjs_kesehatan =
      props.dataLevel.nilai_dasar_potongan_bpjs_kesehatan ?? props.dataLevel.nilai_dasar_potongan_bpjs ?? 0;
    form.nilai_dasar_potongan_bpjs_ketenagakerjaan =
      props.dataLevel.nilai_dasar_potongan_bpjs_ketenagakerjaan ?? props.dataLevel.nilai_dasar_potongan_bpjs ?? 0;
    ...
    form.nilai_dasar_potongan_bpjs_kesehatan = 0;
    form.nilai_dasar_potongan_bpjs_ketenagakerjaan = 0;
    form.id_bpjs_kategori = '';
    form.nilai_point = 0;
...
<template>
  <motion.div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 transition-all">
    <motion.div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-auto p-0 animate-fade-in">
      <motion.div class="px-8 pt-8 pb-2">
        <motion.div class="flex items-center gap-2 mb-6">
        ...
        <form @submit.prevent="submit" class="space-y-5">
          <motion.div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <motion.div>
              ...
              <motion.div v-if="form.errors.nama_level" class="text-xs text-red-500 mt-1">{{ form.errors.nama_level }}</motion.div>
            </motion.div>
            <motion.div>
              ...
              <motion.div v-if="form.errors.nilai_level" class="text-xs text-red-500 mt-1">{{ form.errors.nilai_level }}</motion.div>
            </motion.div>
          </motion.div>
          <motion.div>
          ...
          <motion.div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <motion.div>
              <label class="block text-sm font-medium text-gray-700">Dasar Potongan BPJS Kesehatan (JKN)</label>
              <input v-model="form.nilai_dasar_potongan_bpjs_kesehatan" type="number" min="0" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500" required />
              <motion.div v-if="form.errors.nilai_dasar_potongan_bpjs_kesehatan" class="text-xs text-red-500 mt-1">{{ form.errors.nilai_dasar_potongan_bpjs_kesehatan }}</motion.div>
            </motion.div>
            <motion.div>
              <label class="block text-sm font-medium text-gray-700">Dasar Potongan BPJS Ketenagakerjaan (TK)</label>
              <input v-model="form.nilai_dasar_potongan_bpjs_ketenagakerjaan" type="number" min="0" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500" required />
              <motion.div v-if="form.errors.nilai_dasar_potongan_bpjs_ketenagakerjaan" class="text-xs text-red-500 mt-1">{{ form.errors.nilai_dasar_potongan_bpjs_ketenagakerjaan }}</motion.div>
            </motion.div>
          </motion.div>
          <motion.div>
            ...
            <motion.div v-if="form.errors.id_bpjs_kategori" class="text-xs text-red-500 mt-1">{{ form.errors.id_bpjs_kategori }}</motion.div>
          ...
          <motion.div>
            ...
            <motion.div v-if="form.errors.nilai_point" class="text-xs text-red-500 mt-1">{{ form.errors.nilai_point }}</motion.div>
          </motion.div>

        </form>
      </motion.div>
      <motion.div class="bg-gray-50 px-6 py-4 flex justify-end rounded-b-2xl">

    ...
    <div class="max-w-5xl w-full mx-auto py-8 px-2">
      <div class="flex items-center gap-2 mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2 ml-4">
          ...
          {{ isEdit ? 'Edit Internal Warehouse Transfer' : 'Buat Internal Warehouse Transfer' }}
        </h1>
        <div v-if="isEdit" class="ml-auto">
          <button @click="onDelete" class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition">
            <i class="fa fa-trash mr-2"></i>Hapus
          </button>
        </div>
      </div>

      <form @submit.prevent="onSubmit" class="bg-white rounded-lg shadow-lg p-6 space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select v-model="form.outlet_id" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500" required :disabled="props.user_outlet_id !== 1">
              <option value="">Pilih Outlet</option>
              <option v-for="outlet in props.outlets" :key="outlet.id_outlet" :value="outlet.id_outlet">{{ outlet.nama_outlet }}</option>
            </select>
        ...
        <!-- Serial Mode Toggle -->
        <div class="border rounded-lg p-4" :class="serialMode ? 'border-indigo-300 bg-indigo-50/30' : 'border-gray-200'">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              <i class="fa fa-qrcode text-indigo-500 text-lg"></i>
              <div>
                <div class="font-medium text-gray-800">Mode Nomor Seri</div>
                <div class="text-xs text-gray-500">Aktifkan untuk scan serial number</div>
              </div>
            </div>
        ...
        <div>
          ...
          <div class="mt-4">
            <button type="button" @click="addItem" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition">
              <i class="fa fa-plus mr-2"></i>Tambah Item
            </button>
          </div>
        </div>
        <div class="flex justify-end space-x-4">
          <button type="button" @click="goBack" class="bg-gray-500 text-white px-6 py-2 rounded-lg hover:bg-gray-600 transition">
            Batal
          </button>
          <button type="submit" :disabled="form.processing" class="bg-blue-500 text-white px-6 py-2 rounded-lg hover:bg-blue-600 transition disabled:opacity-50">
...
onMounted(() => {
  if (props.user_outlet_id && props.user_outlet_id !== 1) {
    form.outlet_id = props.user_outlet_id;
  }
});
...
async function fetchItemSuggestions(idx, q) {
  ...
  try {
    const res = await axios.get('/items/search-for-internal-warehouse-transfer', {
...
const canInputItem = computed(() => {
  return form.outlet_id && form.warehouse_outlet_from_id && 
         form.warehouse_outlet_to_id && 
...
async function onSerialScan() {
  ...
  if (!form.outlet_id || !form.warehouse_outlet_from_id) {
    serialFeedback.value = 'Pilih outlet & departemen asal dulu';
  ...
  try {
    const res = await axios.post(route('internal-warehouse-transfer.validate-serial'), {
      serial_number: input,
...
function onSubmit() {
    ...
    for (const [idx, item] of form.items.entries()) {
      ...
      if (unit === item.stock?.unit_small) {
        ...
        if (item.stock?.qty_small && item.stock?.unit_small) {
          ...
          if (unit === item.stock.unit_medium) stock = Math.floor(smallQty / smallConv);
          else if (unit === item.stock.unit_large) stock = Math.floor(smallQty / (smallConv * mediumConv));
          else stock = smallQty;
        }
      }
      
      if (qty > stock) {
        Swal.fire({ icon: 'error', title: 'Gagal!', text: `Qty item "${item.item_name}" melebihi stok di departemen asal untuk unit ${unit} (${stock})` });
        return;
      }
    }
  }
  ...
  // Build payload with serial_items
  const payload = {
    ...form.data(),
  ...
  if (isEdit.value) {
    router.put(`/internal-warehouse-transfer/${props.editData.id}`, payload, {
      onSuccess: () => {
        Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Data berhasil diupdate', timer: 1500, showConfirmButton: false }).then(() => { router.visit('/internal-warehouse-transfer'); });
      },
      onError: () => {
    ...
    router.post('/internal-warehouse-transfer', payload, {
      onSuccess: () => {
        Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Data berhasil disimpan', timer: 1500, showConfirmButton: false }).then(() => { router.visit('/internal-warehouse-transfer'); });
      },
      onError: () => {
...
function onDelete() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(`/internal-warehouse-transfer/${props.editData.id}`, {
        onSuccess: () => {
          Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Data berhasil dihapus', timer: 1500, showConfirmButton: false }).then(() => { router.visit('/internal-warehouse-transfer'); });
        },
        onError: () => {

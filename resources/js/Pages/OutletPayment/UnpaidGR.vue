    ...
    <div class="w-full py-8 px-2">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <i class="fa fa-clock text-red-500"></i> GR Belum Dibuat Payment
        </h1>
      </div>
      <div class="flex flex-wrap gap-2 mb-4 items-center">
        <input v-model="grSearch" @input="onGrFilterChange" type="text" placeholder="Cari outlet atau nomor GR..." class="px-3 py-2 rounded border border-blue-200 shadow focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition w-48" />
      ...
      <div v-if="grGroups && grGroups.data && grGroups.data.length">
        <div v-for="group in grGroups.data" :key="group.outlet_name + group.date" class="mb-6">
          <div class="font-semibold text-blue-700 mb-1">Outlet: {{ group.outlet_name }} | Tanggal: {{ formatDate(group.date) }}</div>
          <table class="w-full border border-gray-200 rounded-xl shadow mb-2">
            <thead>
              <tr class="bg-red-50 text-red-900">
                <th class="px-3 py-2 border">No. GR</th>
                <th class="px-3 py-2 border">Total</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="gr in group.items" :key="gr.id">
                <td class="px-3 py-2 border font-mono">{{ gr.number || gr.gr_number }}</td>
      ...
      <div v-else>
        <div class="text-center text-gray-400 py-4">Tidak ada GR yang belum dibuat payment.</div>
      </div>
    </div>
  </AppLayout>
</template>

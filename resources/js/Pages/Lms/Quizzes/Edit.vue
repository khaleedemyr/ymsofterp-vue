    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden">
      ...
      <div class="relative z-10 py-8 px-6">
        ...
        <div class="backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl shadow-2xl p-6">
          <form @submit.prevent="updateQuiz" class="space-y-8">
...
onMounted(() => {
  form.value = {
    ...
    time_limit_type: props.quiz.time_limit_type || '',
    time_limit_minutes: props.quiz.time_limit_minutes || '',
    time_per_question_seconds: props.quiz.time_per_question_seconds || '',
    passing_score: props.quiz.passing_score || 70,
    max_attempts: props.quiz.max_attempts || '',
    is_randomized: props.quiz.is_randomized || false,
    show_results: props.quiz.show_results !== undefined ? props.quiz.show_results : true,
...
const updateQuiz = async () => {
  ...
  try {
    await router.put(route('lms.quizzes.update', props.quiz.id), form.value, {
      onSuccess: () => {
        Swal.close()
        Swal.fire({
          icon: 'success',
          title: 'Berhasil!',
          text: 'Quiz berhasil diperbarui!'
        })
      },
      onError: (errors) => {
        Swal.close()
        Swal.fire({
          icon: 'error',
          title: 'Error!',
          text: Object.values(errors)[0] || 'Terjadi kesalahan saat memperbarui quiz'
        })
      },
      onFinish: () => {
        loading.value = false
    ...
    Swal.fire({
      icon: 'error',
      title: 'Error!',
      text: 'Terjadi kesalahan saat memperbarui quiz'
    })
  }
}
</script>

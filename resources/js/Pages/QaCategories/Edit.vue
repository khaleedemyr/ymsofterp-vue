...
const form = ref({
  kode_categories: props.category.kode_categories,
...
function submit() {
  ...
  router.put(`/qa-categories/${props.category.id}`, form.value, {
    onSuccess: () => {
...
function cancel() {
  router.visit(`/qa-categories/${props.category.id}`);
}
</script>

<template>
  <AppLayout title="Edit QA Category">
      ...
      <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center gap-4 mb-6">
          ...
          <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <i class="fa-solid fa-edit text-blue-500"></i> Edit QA Category
          </h1>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8">
          <form @submit.prevent="submit" class="space-y-6">
            <!-- Kode Categories -->
            <div>
              <label for="kode_categories" class="block text-sm font-medium text-gray-700 mb-2">
                Kode Categories <span class="text-red-500">*</span>
              </label>
              <input
                id="kode_categories"
                v-model="form.kode_categories"
                type="text"
                ...
                placeholder="Masukkan kode categories"
                required
              />
              <div class="mt-1 text-sm text-blue-600">
                <i class="fa-solid fa-info-circle mr-1"></i>
                Kode categories dapat diedit sesuai kebutuhan
              </div>
              <div v-if="errors.kode_categories" class="mt-1 text-sm text-red-600">
                {{ errors.kode_categories }}
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                type="text"
                class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                placeholder="Masukkan nama categories"
                required
              />
              <div v-if="errors.categories" class="mt-1 text-sm text-red-600">
                {{ errors.categories }}
              </div>
            </div>

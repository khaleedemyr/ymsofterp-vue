...
const form = ref({
  title: props.guidance.title,
  departemen: props.guidance.departemen,
  category_ids: props.guidance.category_ids || [],
  status: props.guidance.status,
  parameters: props.guidance.parameters || [
    {
...
function addParameterPemeriksaan() {
...
function removeParameterPemeriksaan(index) {
  if (form.value.parameters.length > 1) {
    form.value.parameters.splice(index, 1);
  }
}

function addParameterToPemeriksaan(index) {
  const newParam = {
    parameter_pemeriksaan: form.value.parameters[index].parameter_pemeriksaan,
    parameter_id: '',
    point: 0
  };
  form.value.parameters.splice(index + 1, 0, newParam);
}

function removeParameterFromPemeriksaan(index) {
  if (form.value.parameters.length > 1) {
    form.value.parameters.splice(index, 1);
  }
}
...
function submit() {
  ...
  router.put(`/qa-guidances/${props.guidance.id}`, form.value, {
    onSuccess: () => {
...
function cancel() {
  router.visit(`/qa-guidances/${props.guidance.id}`);
}
</script>

<template>
  <AppLayout title="Edit QA Guidance">
      ...
      <div class="max-w-6xl mx-auto">
        ...
        <div class="flex items-center gap-4 mb-6">
          ...
          <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <i class="fa-solid fa-edit text-blue-500"></i> Edit QA Guidance
          </h1>
        </div>

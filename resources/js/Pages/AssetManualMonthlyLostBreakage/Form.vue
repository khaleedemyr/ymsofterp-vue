      ...
      <div class="flex justify-between items-center mb-6">
        <div>
          <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
            ...
            {{ isEdit ? 'Edit' : 'Tambah' }} Asset Manual Monthly Lost & Breakage
          </h1>
          <p class="text-sm text-gray-500 mt-1">Pilih periode, lalu isi data Lost & Breakage per outlet</p>
        </div>
        <Link :href="route('asset-manual-monthly-lost-breakage.index')" class="text-gray-600 hover:text-gray-800">
            ...
            <table class="min-w-full text-sm border border-gray-200">
              <thead class="bg-gray-800 text-white">
                <tr>
                  ...
                  <th class="px-3 py-2 text-right min-w-[160px]">Nilai Lost & Breakage</th>
                  <th class="px-3 py-2 text-right min-w-[140px]">% Lost & Breakage</th>
              ...
              <tbody>
                ...
                <tr v-for="(row, idx) in form.items" :key="row._key" class="border-t align-top">
                  ...
                  <td class="px-3 py-3">
                    <input v-model="row.lost_breakage_value" type="number" step="0.01" min="0" class="w-full rounded-lg border-gray-300 text-right text-sm" placeholder="0" />
                  </td>
                  <td class="px-3 py-3">
                    <input v-model="row.lost_breakage_percent" type="number" step="0.01" min="0" class="w-full rounded-lg border-gray-300 text-right text-sm" placeholder="0" />
                  </td>
...
function makeRow(item = null) {
  return {
    ...
    lost_breakage_value: item?.lost_breakage_value ?? '',
    lost_breakage_percent: item?.lost_breakage_percent ?? '',
  }
}
  ...
  const payload = {
    ...
    items: form.items
      ...
      .map((row) => ({
        ...
        lost_breakage_value: row.lost_breakage_value === '' ? 0 : row.lost_breakage_value,
        lost_breakage_percent: row.lost_breakage_percent === '' ? 0 : row.lost_breakage_percent,
      })),
  }

  if (isEdit.value) {
    form.transform(() => payload).put(route('asset-manual-monthly-lost-breakage.update', props.record.id))
  } else {
    form.transform(() => payload).post(route('asset-manual-monthly-lost-breakage.store'))
  }
}
</script>

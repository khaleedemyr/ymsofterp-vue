            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-5xl mx-4 max-h-[90vh] overflow-y-auto" @click.stop>
                ...
                <div v-else class="space-y-2">
                    <div v-for="po in paginatedApprovals" :key="'all-po-' + po.id"
                         ...
                         class="p-3 rounded-lg cursor-pointer transition-all duration-200 hover:scale-[1.02] border border-gray-200 dark:border-gray-700 hover:border-indigo-500 dark:hover:border-indigo-500"
                         :class="isNight ? 'bg-slate-700/50 hover:bg-slate-600/50' : 'bg-gray-50 hover:bg-indigo-50'">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                <div class="font-semibold text-sm" :class="isNight ? 'text-white' : 'text-slate-800'">
                                    {{ po.number }}
...
async function approvePO() {
    ...
    try {
        ...
        closeDetailModal();
        loadPendingApprovals();
        emit('approved', selectedPO.value.id);
    } catch (error) {
        console.error('Error approving PO Food:', error);
        Swal.fire('Error', error.response?.data?.message || 'Gagal menyetujui PO Food', 'error');
    }
}
    ...
    Swal.fire({
        ...
        if (result.isConfirmed && selectedPO.value) {
            try {
                ...
                closeDetailModal();
                loadPendingApprovals();
                emit('rejected', selectedPO.value.id);
            } catch (error) {
                console.error('Error rejecting PO Food:', error);
                Swal.fire('Error', error.response?.data?.message || 'Gagal menolak PO Food', 'error');
            }
        }
    });
}

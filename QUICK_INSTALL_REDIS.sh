...
if [ -f "$PHP_BIN" ]; then
    if $PHP_BIN -m | grep -q redis; then
        ...
        # Try to install via yum first
        if yum install -y "ea-php${PHP_VERSION}-php-redis" 2>/dev/null; then
            echo "✅ PHP Redis extension installed via yum"
        else
...
echo "✅ Redis port: $(netstat -tlnp 2>/dev/null | grep 6379 | awk '{print $4}' || echo 'Not listening')"
echo ""
